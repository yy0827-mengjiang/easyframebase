...
function rePosition(){
	...
	if($("#frame_manager").offset() != undefined){
		var offset = $("#frame_manager").offset();
		$("#managerWindow").css({position:"absolute",right:($(window).width()-offset.left-50),top:(offset.top+22)});
	}
	if($("#frame_serach").offset() != undefined){
		var offset = $("#frame_serach").offset();
		$("#searchWindow").css({position:"absolute",right:($(window).width()-offset.left-50),top:(offset.top+22)});
	}
...
$(function(){
	...
	$("#frame_serach").bind("click", function(e){
	   $('#searchWindow').css({top:(e.pageY+15)});
	   $('#searchWindow').show();
	   //$("#insert_collect").linkbutton();
	});
