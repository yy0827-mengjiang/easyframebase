...
function leftContextMenu(e,node){
    ...
    if(node.attributes.type=="compositeKpiCategory"){
    	...
    	if(node.attributes.data_type == '2'||node.attributes.data_type == '3') {
    		if(node.attributes.data_type == '2'){
    			$('#compositeKpiNodeMenu').menu('show',{left:e.pageX,top:e.pageY});
    		}
    		if(node.attributes.data_type == '3'){
...
function coverage(){
	 ...
	 if(selected){
			$("#kpi").html("");
		    $("#kpi").load('../relationship/relationshipFrame.jsp?kpi_key='+selected.attributes.code+"&kpi_name="+selected.text+"&kpi_version="+selected.attributes.version+"&kpiId="+selected.id+"&kpi_type="+selected.attributes.kpi_type,'',function(){
			   kpiDivTitle();
		    });
//	        window.open('../relationship/relationshipFrame.jsp?kpi_key='+selected.attributes.code+"&kpi_version="+selected.attributes.version+"&kpi_name="+selected.text,'影响范围', 'height=480, width=1000, top=60, left=300, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
	    }
}
