...
<e:q4o var="dimObj">
	SELECT ID,DIM_CODE,
		  ...
		  DIM_CATEGORY
	  FROM X_KPI_DIM_CODE
	 where DIM_CODE = '${param.dim_code }'
</e:q4o>
<e:q4o var="cateGoryName">
	SELECT T.CATEGORY_NAME FROM X_KPI_CATEGORY T WHERE T.CATEGORY_ID = '${dimObj.DIM_CATEGORY }'
</e:q4o>
<e:q4l var="dimList">
	SELECT DIM_CODE, CODE_TABLE,CODE_TABLE_DESC, COLUMN_CODE, COLUMN_DESC, COLUMN_ORD,COLUMN_PARENT FROM X_KPI_DIM_CODE WHERE DIM_CODE !='${param.dim_code }' 
</e:q4l>
...
$(function() {
	$('input.checkN').iCheck({
	 ...
	 $("#dim_category_hid").val('${dimObj.DIM_CATEGORY}');
	...
	function save(){
		...
		if(validate()){
			$.messager.confirm('请确认','您确定要修改当前维度信息吗?',function(msg){
				if(msg) {
					$.post('<e:url value="/pages/kpi/basedim/dimManagerAction.jsp?eaction=editDim"/>',info, function(data){
			            var temp = $.trim(data);
			           // alert(temp);
			            if(temp=='1'){
...
<!-- 		<dt>维度编码：</dt> -->
			<dd>
				...
				<input type="hidden" id="id" name="id" value="${dimObj.ID }" readonly="true"/>
<!-- 				<span id="tip" style="color:red;" ></span> -->
		...
		<div id="dimType">
			...
			<dl class="fourD">
				...
				<dt>维度分类：</dt>
				<dd>
					<input type="text" id="dim_category" name="dim_category" readonly="readonly"  value="${cateGoryName.CATEGORY_NAME }">
