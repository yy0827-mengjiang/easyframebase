...
function formulaContextMenu(e,node) {
	  e.preventDefault();
	  $(this).tree('select', node.target);
  	  $('#formulaNodeMenu').menu('show',{left:e.pageX,top:e.pageY});
}
function formulaNodeDelete() {
	var selected=$('#baseformula').tree('getSelected');
    if(selected){
		  $.messager.confirm('确认信息','您确认要删除当前公式信息吗?',function(r){
		 		if(r){
		 			$.post(appBase + "/pages/kpi/formulaKpi/formulaAction.jsp",{eaction:"delFormul",id:selected.id},
		 	    			  function(data){
						$.messager.alert("提示信息！", "公式信息删除成功！", "info",function(){
						    $("#baseformula").tree("options").url= appBase + '/pages/kpi/formulaKpi/formulaAction.jsp?eaction=formula';
						    $("#baseformula").tree("reload");
						});
		 	    	}, "json");
		 		}
		  });
        ...
        onDrop:function(e,s){
        	...
        	var isleaf = $(this).tree('isLeaf',node.target);
         	if(isleaf){
    			...
    			if(!vaildKpiType(_usedFormula,_formulaName,_typeName,node.attributes.kpi_type)){
    				var _text = "";
    				for(var i=0;i<_formulaName.length;i++){
    					if(_text.length>2){
    						_text += "，";
    					}
    					_text += _formulaName[i].TYPE_NAME;
    				}
...
function setTdHtml(o2,o,o1){
	...
	if(_dimAttr){
		...
		if(_dimAttr=="T"||"R" == _dimAttr){
			...
			$(o).append("<input type='text' value='"+_value+"' attrName='nodeId' style='width:300px;'>");
		}
	}else{
...
//	}
	...
	if(_name.length>100){
		$.messager.alert("提示信息！", '指标名称超长 ！', "info");
		clearArg();
		enbBut();
		return false;
	}
	...
	if(_kpi_nuit == "" || null == _kpi_nuit) {
		$.messager.alert("提示信息！", '请选择指标单位！', "info");
		clearArg();
		enbBut();
		return false;
	}
	...
	if(_business.length > 4000){
		$.messager.alert("提示信息！", '业务口径超长！', "info");
		clearArg();
		enbBut();
		return false;
	}
	...
	if(_technical.length > 4000){
		$.messager.alert("提示信息！", '技术口径超长 ！', "info");
		clearArg();
		enbBut();
		return false;
	}
	...
	if(_dimAuthor.length > 8){
		$.messager.alert("提示信息！", '提出者超长 ！', "info");
		clearArg();
		enbBut();
		return false;
	}
	...
	if(_dimDept.length > 100){
		$.messager.alert("提示信息！", '提出部门超长！', "info");
		clearArg();
		enbBut();
		return false;
	}
...
function closeFormula(){
	$.messager.confirm('提示信息', '确认关闭当前页面?', function(r){
		if (r){
			$('#kpi').html('');
			$('#kpi').load('../kpiManager/kpiMain.jsp');
		}
	});
}
function kpiType(){
...
function acctTypeAle1(acctType,sub){
	cn.com.easy.kpi.service.FormulaService.kpiAttr(acctType,function(data){
		var _data = $.parseJSON(data);
		var _sub = $.parseJSON(sub);
		if(_data.res=="success"){
			...
			for(var i=0;i<_value.length;i++){
				if(_value[i].ischeck=='1'){
					_acctType.append("<span>"+_value[i]['text']+"</span>");
				}
			}
			var _falg = disExtA("tid");
			if(_falg&&_type!='1'){
...
//				$('#'+id).append("<option value='"+_value[i]['code']+"'>"+_value[i]['text']);
			...
			if(_value.length>0){
				...
				$('#'+kpiIndex).attr("nodeid",kpi_code);
				$('#'+kpiIndex).html(text+"<strong>X</strong>");
