...
<e:if condition="${typeInfo.USED_TYPE ne ''&&typeInfo.USED_TYPE ne null }">
	<e:q4l var="userType">
		SELECT TYPE_NAME AS "TYPE_NAME" FROM X_KPI_TYPE T WHERE CAST(T.TYPE_CODE AS INTEGER) IN (${typeInfo.USED_TYPE})
	</e:q4l>
</e:if>
<e:if condition="${typeInfo.FORMULA ne '' && typeInfo.FORMULA ne null }">
	<e:q4l var="formulaName">
		SELECT TYPE_NAME AS "TYPE_NAME"  FROM X_KPI_TYPE T WHERE CAST(T.TYPE_CODE AS INTEGER) IN (${ypeInfo.FORMULA})
	</e:q4l>
</e:if>


<e:q4l var="unit" sql="kpi.view.unit"></e:q4l>
<e:q4l var="opter" sql="kpi.formulaKpi.opter"></e:q4l>
<e:q4l var="logical" sql="kpi.formulaKpi.logical"></e:q4l>

<e:q4l var="kpiType" sql="kpi.view.kpiType"></e:q4l>
<e:q4l var="serviceType" sql="kpi.view.serviceType"></e:q4l>
<e:q4l var="classification" sql="kpi.view.classification"></e:q4l>
<e:q4l var="cycle" sql="kpi.view.cycle"></e:q4l>
<e:q4l var="reservedAttr" sql="kpi.view.reservedAttr"></e:q4l>
<!DOCTYPE>
<html>
<head>
...
<script type="text/javascript">
	var _accountType = '${cubeInfo.ACCOUNT_TYPE}';
	var _type = '${typeInfo.TYPE_CODE}';
	//var _type='1';
	//生成指标CODE
	var comCode = 'FK_' + ${sysCode };
	//生成逻辑符下拉框
	...
	var _usedFormula = '${typeInfo.FORMULA}';
	if(_usedFormula==null||_usedFormula==''){
		_usedFormula = _usedType;
	}
	 var _typeName = '${typeInfo.TYPE_NAME}';
	 /*var _usedName = $.parseJSON('${e:java2json(userType.list)}'); */
	<e:if condition="${typeInfo.USED_TYPE != null && typeInfo.USED_TYPE !='' }" >
		_usedName = $.parseJSON('${e:java2json(userType.list)}');
	</e:if>
	<e:if condition="${typeInfo.FORMULA != null && typeInfo.FORMULA !='' }" var="isnull">
		_formulaName = $.parseJSON('${e:java2json(formulaName.list)}');
	</e:if>
	<e:else condition="${isnull }">
		_formulaName=$.parseJSON('${e:java2json(userType.list)}');
	</e:else>
	$(function() {
		$('#formulaBut').hide();
		$('#ysArea').hide();
		...
		$('#kt').val('${param.server_class}');
		var _classNum = $.parseJSON('${e:java2json(classNum.list)}');
		for (var z = 0; z < _classNum.length; z++) {
			$('#' + _classNum[z].CLASS_NAME + "Val").html(
					$('#' + _classNum[z].CLASS_NAME).find("option:selected")
							.val());
		}
		//给公式加事件
		formulaTD();
				...
				.droppable(
						{
							onDragEnter : function(e, source) {
								$(source).draggable("proxy").find(
										"span.tree-dnd-icon").removeClass(
										"tree-dnd-no tree-dnd-no").addClass(
										"tree-dnd-yes");
							},
							//如果拖拽的是指标，约束条件表格中“条件值”为input，如果是维度约束条件表格中“条件值”通过“连接符”来确定，连接符>、<、=、是select，is null、is not null为空，in为一个弹出窗，可以多选。
							onDrop : function(e, s) {
								//获取约束条件Table的总行数
								var index = $(".condTab tr").length;
								//生成约束条件中指标的账期下拉框
								var __type = "<select id='sel"
										+ index
										+ "' onchange=\"selectType('sel"
										+ index
										+ "')\"><option value=''>--请选择--</select>";
								//生新约整条件中运算符下拉框
...
// 								console.log(node.target);
								...
								if (isleaf) {
									...
									if (_dropNode == 'KPI') {
										if (!vaildKpiType(_usedType, _usedName,
												_typeName,
												node.attributes.kpi_type)) {
											var _text = ""
										...
										if (_dropDate == '3') {
											...
											if (node.attributes.kpi_type != '5') {
												_tr = '<tr><td  name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td id="'+node.attributes.code+'_'+index+'" name="nodeId" nodeId="'+node.attributes.code+'" dimType="">'
														+ node.text
														+ '</td><td name="oper" nodeId="0">'
														+ _select
														+ '</td><td name="condval" id="cond'+index+'" nodeId="0" condAttr="" attrVal=""><input type="text" value="0" style="width:300px"  attrName="nodeId"></td></tr>';
											} else {
												//标签中条件值只有1和0
												var _labeSelect = '<select style="width:400px" attrName="nodeId"><option value="1">是</option><option value="0">否</option></select>';
												//生成标签的tr
												_tr = '<tr><td name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td nodeId="'+node.attributes.code+'" name="nodeId" dimType="">'
														+ node.text
														+ '</td><td  name="oper" nodeId="0">'
														+ _select
														+ '</td><td name="condval" id="cond'+index+'" nodeId="1" " condAttr="" attrVal="" >'
														+ _labeSelect
														+ '</td></tr>';
											}
											//将TR追加到table
											...
											if (node.attributes.kpi_type == '5') {
												...
												//生成一个tr
												_tr = '<tr><td  name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td id="'+node.attributes.code+'_'+index+'" name="nodeId" nodeId="'+node.attributes.code+"_A"+'" dimType="">'
														+ node.text
														+ __type
														+ '</td><td name="oper" nodeId="0">'
														+ _select
											...
											$(this).append(_tr);
											//通过结点的数据获取指标的类型
											acctTypeSub(node.text,
													node.attributes.code,
													node.attributes.version,
													node.attributes.account,
													node.attributes.code + "_"
															+ index, 'sel'
															+ index);
										...
										if ('R' == _dimAttr||'T'==_dimAttr) {
											_tr = '<tr><td name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td nodeId="'+node.attributes.code+'" name="nodeId" dimType="'+node.attributes.kpi_type+'">'
													+ node.text
													+ '</td><td  name="oper" nodeId="0">'
													+ _select
													+ '</td><td name="condval" id="cond'
													+ index
													+ '" nodeId="" condAttr="'
													+ _dimAttr
													+ '" attrVal="" ondblclick="openCondValue(\'cond'
													+ index
													+ '\')">'
													+ _selectVal + '</td></tr>';
										} else {
											_tr = '<tr><td name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td nodeId="'+node.attributes.code+'" name="nodeId" dimType="'+node.attributes.kpi_type+'">'
													...
													+ '" nodeId="" condAttr="'
													+ _dimAttr
													+ '" attrVal="0"  ondblclick="openCondValue(\'cond'
													+ index
													+ '\')">'
													+ _selectVal
													+ '&nbsp;<input type="hidden" style="width:300px" attrName="attrVal" value="0"></td></tr>';
										}
										//将生成的tr追加到约束条件table中
										$(this).append(_tr);
										//维度默认下拉框值
										CondValue(node.attributes.code, "cond"
												+ index);
										//为下拉框绑定事件
										selectVal();
										//为checkbox绑定事件
										...
										str += mydate.getDate();
										if ('R' == _dimAttr||'T'==_dimAttr) {
											_tr = '<tr><td name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td nodeId="'+node.attributes.code+'" name="nodeId" dimType="'+_dropNode+'">'
													+ node.text
													+ '</td><td  name="oper" nodeId="0">'
													+ _select
													+ '</td><td name="condval" id="cond'+index+'" nodeId="0" condAttr="' + _dimAttr + '" attrVal="" ><input type="text" value="" style="width:300px" attrName="nodeId"></td></tr>';
										} else {
											_tr = '<tr><td name="nodeType" nodeID="'+node.attributes.kpi_type+'"><input type="checkbox" name="condCB" id="'+index+'" nodeId="'+node.id+'"></td><td nodeId="'+node.attributes.code+'" name="nodeId" dimType="'+_dropNode+'">'
													+ node.text
													+ '</td><td  name="oper" nodeId="0">'
													+ _select
													+ '</td><td name="condval" id="cond'+index+'" nodeId="' + str + '" condAttr="' + _dimAttr + '" attrVal="0" ><input type="text" value="' + str + '" style="width:300px"  attrName="nodeId">&nbsp;<input type="hidden" style="width:300px"  attrName="attrVal" value="0"></td></tr>';
										}
										//将生成的tr追加到约束条件table中
										$(this).append(_tr);
										//为input绑定事件
	...
	function addLogical() {
		...
		var _tr = '<tr><td name="nodeType" nodeId="0"><input type="checkbox" name="condCB" id="'+index+'"></td><td name="nodeId" nodeId="">逻辑符</td><td name="oper" nodeId="and">'
				+ _select + '</td><td name="condval" nodeId=""></td></tr>';
		var _dat;
		var _checked = $("input[name='condCB']");
		...
		if (null != _dat) {
			var _before = _dat.parents('tr');
			_before.before(_tr);
			_dat.attr("checked", false);
				...
				.alert(
						...
						function() {
							if (flag != "1") {
								...
								$("#kpi")
										.load(
												'<e:url value="/pages/kpi/kpiManager/kpiMain.jsp"/>?kpi_category=&cube=${param.cubeCode}');
								$('#tt').tree(
										'reload',
										$('#tt').tree('getParent',
												current_node.target).target);
								//window.location.href='../../../pages/kpi/kpiManager/newKpiManager.jsp?account_type='+_accountType + '&managerFlag=${param.managerFlag}';
							}
						});
	}
...
<body>
	<input type="hidden" id="SEL_CUBE_CODE" name="SEL_CUBE_CODE"
		value="${param.cubeCode }" />
	<input type="hidden" id="operFlag" name="operFlag" value="add" />
	<div class="kpi_guide" id="formKpi">
		<div class="tit_div1">
			<h3>指标信息</h3>
			<span> <!-- <button onclick="draft('draft')">保存草稿</button> -->
				<button onclick="formulasaved('0')" id="formulasaved">提交版本</button>
		...
		<div class="editBase_div1">
			<form id="formulaForm" enctype="multipart/form-data">
				...
				<input type="hidden" name="parentId" id="parentId" value="${param.kpi_category }"> 
				<input type="hidden" name="types" id="types" value="${param.baseType }"> 
				<input type="hidden" name="cubeCode" id="cubeCode" value="${param.cube_code }"> 
				...
				<input type="hidden" name="formKpi_type" id="formKpi_type" value="${cubeDetail.account_type }"> 
				<input type="hidden" name="formKpiSub" id="formKpiSub"> 
				<input type="hidden" name="base_key" id="base_key" readonly="readonly" value="FK_${sysCode }"> 
				<input type="hidden" name="formula_version" id="formula_version" readonly="readonly" value="${version }">
				<div class="editBase_div_child1">
							...
							<dd class="kpiTableArea">
								<div class="kpiTrTop">
									<e:forEach items="${classNum.list }" var="class">
										<span class="kpiCodeCol" id="${class.CLASS_NAME }Val" width="${100/classCount.C }%"> </span>
									</e:forEach>
								</div>
								<table>
									<tr class="kpiTrBot">
										<e:forEach items="${classNum.list }" var="class" indexName="index">
											<td width="${100/classCount.C }%">
												<select id="${class.CLASS_NAME }" codeStart="${index*2}" name="${class.CLASS_NAME }" style="width: 100%;">
													<e:q4l var="kpiType" sql="kpi.formulaKpi.kpiType">
								...
								<div id="acctType">
									<e:forEach items="${formulaType.list}" var="fl" indexName="ind">
										<e:if condition="${fl.ATTR_CODE eq 'A'}" var="flag">
											<input type="checkbox" name="formKpiTypes" value="${fl.ATTR_CODE }" checked='checked'>${fl.ATTR_NAME }
										</e:if>
										<e:else condition="${flag }">
											<input type="checkbox" name="formKpiTypes" value="${fl.ATTR_CODE }">${fl.ATTR_NAME }
										</e:else>
									</e:forEach>
								</div>
							</dd>
					...
					<dl class="fourD">
						<dt>指标名称：</dt>
						<dd>
							<input type="text" name="formula_name" id="formula_name" onblur="validName()">
							<B style="color: red;">&nbsp;*</B>
						</dd>
						<dt>指标单位：</dt>
						<dd>
					...
					<dl class="ddLine">
						<dt>业务口径：</dt>
						<dd>
							<textarea rows="3" cols="100" name="business" id="business" onblur="formatSql('business')"></textarea>
							<B style="color: red;">&nbsp;*</B>
						</dd>
					</dl>
					<dl class="ddLine">
						<dt>技术口径：</dt>
						<dd>
							<textarea rows="3" cols="100" name="technical" id="technical" onblur="formatSql('technical')"></textarea>
						</dd>
					</dl>
					<div class="kpiDivider"></div>
					<dl class="fourD">
					...
					<dl class="ddLine">
						<dt>需求来源：</dt>
						<dd>
							<textarea rows="3" cols="100" id="explain" name="explain"></textarea>
							<br> 
					...
					<tr>
						...
						<td>
							<p id="tid" class="boxTextP">
								<span class='box_div formulaTD'>&nbsp;</span>
							</p>
						</td>
					</tr>
				</tbody>
	...
	<div id="ysArea" style="display:none;">
		<div class="tit_div">
			<h3>约束条件</h3>
			<span>
	...
	<!-- 选择公式-->
	<div id="formula-dlg" class="easyui-dialog" title="选择公式" style="width:650px;height:480px;" data-options="closed:true,modal:true">
	</div>
	...
	<div id="f-dlg" class="easyui-dialog" title="公式配置" style="width:760px;height:450px;" data-options="closed:true,modal:true" buttons='#dlg-buttons1'>
		<table class="pageTable">
			...
			<tr>
				<th>定义公式：</th>
				<td>
					<textarea id="kpi_formula" name="kpi_formula" onselect="setCaret(this);" onclick="setCaret(this);" onkeyup="setCaret(this);" onfocus="setCaret(this);" style="width:99%;height:60px;"></textarea>
				</td>
			</tr>
			<tr>
				<th>运算符：</th>
			...
			<tr>
				<th>表达式：</th>
				<td><select id="kpi_bds" name="kpi_bds" multiple="multiple" style="width: 600px; height: 150px; margin-top: 10px; cursor: pointer; " ondblclick="javascrtip:doBds(this);">
	...
	<!-- 保存时弹出的指标类型 -->
	<div id="formulaType" class="easyui-dialog" title="指标类型" style="width:400px;height:150px; text-align:center;" data-options="closed:true,modal:true" buttons="#dlg-buttons2">
		<div id="acctType">
