...
<html>
  ...
  <div class="kpi_guide">
  	<div class="tit_div1">
		<h3>${kpi.KPI_NAME }[V${kpi.KPI_VERSION }]</h3>
		<span>
			<e:if condition="${lookUpFlag == '1'}">
           		<button type="button" name="close" onclick="doCloseKpi()">关闭</button>
			</e:if>
		</span>
	</div>
	...
	<form id="formulaForm" enctype="multipart/form-data">
	   ...
	   <dl class="ddLine">
			<dt>技术口径：</dt>
			<dd><textarea class="taStyleNone" id="technical" disabled="disabled">${kpi.KPI_CALIBER }</textarea>
			</dd>
		</dl>
		<dl class="ddLine">
			<dt>业务口径：</dt>
			<dd><textarea class="taStyleNone" id="business" disabled="disabled">${kpi.KPI_EXPLAIN }</textarea>
			</dd>
		</dl>
	    <div class="kpiDivider"></div>
		<dl>
			<dt>指标单位：</dt>
			<dd>${kpi.KPI_NUIT_NAME }
			</dd>
		</dl>
