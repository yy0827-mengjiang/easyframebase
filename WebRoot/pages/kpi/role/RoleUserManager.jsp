...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function deleteUser(userId){
				...
				$.post('<e:url value="/pages/kpi/role/RoleAction.jsp?eaction=removeUser"/>', params, function(data){
					if($.trim(data)>0){
			...
			function addUser(userId){
				...
				$.post('<e:url value="/pages/kpi/role/RoleAction.jsp?eaction=addUser"/>', params, function(data){
	             	if($.trim(data)=='1'){
	             		$('#hasSelectUsersTable').datagrid("load",$("#hasSelectUsersTable").datagrid("options").queryParams);
	             		$('#needSelectUsersTable').datagrid("load",$("#needSelectUsersTable").datagrid("options").queryParams);
	             	}else{
	...
	<div style="width:50%; height:100%;float:left;">
		...
		<c:datagrid url="/pages/kpi/role/RoleAction.jsp?eaction=getHasSelectUsers&currentNodeId=${param.currentNodeId }" id="hasSelectUsersTable" pageSize="10" toolbar="#tb1" fit="true" border="false">
			<thead>
				<tr>
	...
	<div style="width:50%; height:100%;float:left;">
		...
		<c:datagrid url="/pages/kpi/role/RoleAction.jsp?eaction=getNeedSelectUsers&currentNodeId=${param.currentNodeId }" id="needSelectUsersTable" pageSize="5" fit="true" toolbar="#tb2">
			<thead>
				<tr>
					<th field="LOGIN_ID" width="200">
						登陆ID
