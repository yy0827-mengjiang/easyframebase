...
<e:set var="kpi_category">${param.kpi_category }</e:set>
<e:q4l var="unitList">select code,name from x_kpi_code where type='0'</e:q4l>
<e:set var="unitJson">${e:java2json(unitList.list) }</e:set>
...
<e:script value="/resources/component/easyui/jquery.easyui.min.js" />
...
<body id="underLine">
	<div class="easyui-panel newAddkpi" title="编辑指标">
		<div class="comOutBox">
			<!--titOut-->
			<div class="titOut">
				<h3>
			...
			<!--comInbox-->
			<div class="comInbox">
				<input type="hidden" id="oid" name="oid" value="${oid }"> <input
					type="hidden" id="code" name="code" value="${code }"> <input
					type="hidden" id="kpiVersion" name="kpiVersion"
					value="${KPIVERSION }"> <input type="hidden"
					id="kpiCategory" name="kpiCategory" value="${kpiCategory }">

				<dl class="group">
					<dt class="topLine">指标名称：</dt>
					<dd>
						<input type="text" id="kpi_name0" name="kpi_name0" width="45%"
							style="height: 22px;" class="noSearch" value="${name }">
						<input type="hidden" id="isFormula" name="isFormula" value="1">
						<span class="pl6">指标单位：</span><select id="kpi_unit0"
							name="kpi_unit0" value="${unit }"></select>
					</dd>
				</dl>
				<dl>
					<dt>指标公式：</dt>
					<dd>
						<div id="formKpi" myAttr="formkpi" class="formEdit"
							onclick="saveRange();clickArea(window.event,'formKpi')"
							onkeydown="saveRange();" onkeyup="saveRange();" tabIndex='-1'>
							${fm }</div>
					</dd>
				</dl>
			</div>
			...
			<div class="comInbox">
				<dl class="group">
					<dt>指标口径：</dt>
					<dd>
						<textarea rows="3" id="kpiCalIber" name="kpiCalIber"
							style="width:100%">${kpiCalIber }</textarea>
					</dd>
				</dl>
				<dl class="group">
					<dt>指标解释：</dt>
					<dd>
						<textarea rows="3" id="kpiExplain" name="kpiExplain"
							style="width: 100%">${kpiExplain }</textarea>
					</dd>
				</dl>
			...
			<div div class="comInbox">
				<p class="titleSearch">
					指标提出人：<input type="text" id="kpiUser" name="kpiUser"
						value="${KPIUSER }"> 指标提出部门：&nbsp;<input type="text"
						id="kpiDept" name="kpiDept" value="${KPIDEPT }">
				</p>
				<p class="titleSearch">
					指标需求：
					<textarea rows="3" id="kpiFile" name="kpiFile" style="width: 80%">${KPIFILE }</textarea>
				<p>
			</div>
			...
			<div class="centerBtnItem">
				<p>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok"
						onclick="onAgree3()">保存当前版本</a> <a href="javascript:void(0)"
						class="easyui-linkbutton" iconCls="icon-ok" onclick="onAgree2()">发布新版本</a>
				</p>
			</div>
		</div>
