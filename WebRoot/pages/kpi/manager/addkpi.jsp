...
<script type="text/javascript">
	...
	$(function() {
		...
		for(var i=0;i<unit.length;i++){
			vUnit.append("<option value='"+unit[i].CODE+"'>"+unit[i].NAME+"</option>");
		}
	});
	...
	function chooseKpiOrFormula() {
		$('#base_kpi').load('<e:url value="/pages/kpi/manager/kpi.jsp"/>',{},function(){});
	...
	function chooseDim() {
		$('#dim-dlg').css('display','');
		$('#dimDIV').load('<e:url value="/pages/kpi/manager/dim.jsp"/>',{},function(){});
		$('#kpiDIV').load('<e:url value="/pages/kpi/manager/kpiCon.jsp"/>',{},function(){});
		$('#dim-dlg').dialog('open');
	}
	function chooseTjz() {
		...
		if('0'==isConstant){
			...
			$.ajax({
	             type: "POST",
	             url: "<e:url value='/pages/kpi/manager/kpiAction.jsp?eaction=dimensionForId'/>&oid="+dimCode+"&tjzId="+tjzId,
	             dataType: "json",
	...
	function insertContent(textObj,filedValue) {
		if (document.all) {
            if (textObj.createTextRange && textObj.caretPos) {
                ...
                caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == '   ' ? filedValue + '   ' : filedValue;
            } else {
	...
	function doLockFormula() {
		...
		if(lockstatus == "解锁公式") {
			...
			$("#ysf8").val("解锁公式");
			disabled = true;
			var param = {"index": 0, "row":{"kpi_forms":formula}};
			$("#formTable").datagrid('updateRow',param);
	...
	function addkpi(){
		...
		$('#conditionTable').datagrid('insertRow',
												  {index:indexs,row:{"dimName":"<input type='text' id='dim_name"+indexs+"' name='kpi_name"+indexs+"' style='width:100%;' ondblclick='javascript:chooseDim();' readonly='readonly'><input type='hidden' id='dimName"+indexs+"' name='kpiName"+indexs+"'><input type='hidden' id='isKpi"+indexs+"' name='isKpi"+indexs+"' value='1'>",
	                                                            "ljf":"<select id='ljf_name"+indexs+"' name='kpi_name"+indexs+"' style='width:100%;' onchange='onMultiple()'><option value='0'>等于</option><option value='1'>包含</option><option value='2'>大于</option><option value='3'>小于</option><option value='4'>不等于</option><option value='5'>大于等于</option><option value='6'>小于等于</option><option value='7'>为空</option><option value='8'>不为空</option></select>",
	                                                            "tjz":"<input type='text' id='tjz_unit"+indexs+"' name='kpi_unit"+indexs+"' style='width:100%;' ondblclick='javascript:chooseTjz()'  readonly='readonly'><input type='hidden' id='did"+indexs+"' name='did"+indexs+"' ><input type='hidden' name='hidconstant"+indexs+"' id='hidconstant"+indexs+"'>",
	                                                            "ljkjf":"<select id='ljkjf"+indexs+"' name='ljkjf"+indexs+"' style='width:100%;'><option value='and'>并且</option><option value='or'>或者</option></select>"}
												  });
	...
	function onAgree2(){
		...
		agr.kpiUnit1 = $('#kpi_unit1').val();//复合指标公式（替换名称后）
		agr.formual = $('#hidForms').val();//复合指标公式（替换ID后）
		agr.isFormula = $('#isFormula').val();//是否公式
		agr.dIndex = $('#dIndex').val();//条件个数
		agr.kpiCode = $('#kpiCode').val();//标准指标ID
		agr.complexkpiCode = $('#complexkpiCode').val();//复合指标ID
		agr.kpi_category = '${parentId}';//分类
	...
	function onCheckKpiName(){
		...
		$.ajax({
			 type: "POST",
             url: "<e:url value='/pages/kpi/manager/ajax.jsp?eaction=name'/>&name="+kpiName+"&comKpiCode="+comKpiCode,
             dataType: "json",
             success: function(data){
             	if(data.C>0){
             		Vcheck="3";
...
<body id="underLine">
		...
		<div class="comOutBox">
			...
			<!--comInbox-->
			<div class="comInbox">
				<input type="hidden" id="kpiCode" name="kpiCode">
				<input type="hidden" id="complexkpiCode" name="complexkpiCode">
				<input type="hidden" id="hidForms" name="hidForms">
				<input type="hidden" id="kpiVersion" name="kpiVersion" value="0">
				<table width="100%">
				<c:datagrid url="#" id="formulaTable" pagination="false" rownumbers="true" >
					<thead>
						<tr>
					  ...
					  <tbody>
		       	 		<tr >
		       	 			<td><input type="text" id="kpi_name0"  name="kpi_name0" style="width:90%; "  class="noSearch" onblur="onCheckKpiName()"><input type="hidden" id="isFormula" name="isFormula" value="1"></td>
		       	 			<td><!-- <input type="text" id="kpi_unit0"  class="underLine" name="kpi_unit0" style="width:100px;" onblur="onKpiUnit()"> -->
		       	 				<select id="kpi_unit0" name="kpi_unit0" style="width:100%;">
		       	 				</select>
		       	 			</td>
		       	 			<td><input type="text" id="kpi_unit1"  class="underLine" name="kpi_unit1" style="width:90%;"  ondblclick="javascript:chooseKpiOrFormula();" readonly="readonly"></td>
		       	 		</tr>
		       	 	</tbody> 
				</c:datagrid>
				</table>
		...
		<!--comInbox-->
		<div class="comInbox">
					<input type="hidden" name="dIndex" id="dIndex" value="0"/><!-- 条件id -->
					<table width="100%">
					<c:datagrid url="#" id="conditionTable" pagination="false" rownumbers="true" >
						<thead>
							<tr>
						  ...
						  <tbody  class="underLine">
			       	 		<tr>
			       	 			<td><input type="text" id="dim_name0" name="kpi_name0" style="width:90%;" ondblclick="javascript:chooseDim();" readonly="readonly">
			       	 				<input type='hidden' id='dimName0' name='dimName0'><input type="hidden" id="isKpi0" name="isKpi0">
			       	 			</td>
			       	 			<td><select id="ljf_name0" name="kpi_name0" style="width:90%;" onchange="onMultiple()">
					       	 			...
					       	 			<option value='7'>为空</option><option value='8'>不为空</option>
			       	 			</select></td>
			       	 			<td><input type="text" id="tjz_unit0" name="kpi_unit0" style="width:90%;" ondblclick="javascript:chooseTjz()" readonly="readonly">
			       	 				<input type="hidden" name="hidconstant0" id="hidconstant0">
			       	 			</td>
			       	 			<td><select id="ljkjf0" name="ljkjf0" style="width:90%;"><option value="and">并且</option><option value='or'>或者</option></select>
			       	 				<input type='hidden' id='did0' name='did0' >
			       	 			</td>
			       	 		</tr>
			       	 	</tbody>
					</c:datagrid>
		...
		<div class="comInbox">
			<dl class="group">
				<dt>指标口径：</dt>
				<dd>
					<textarea rows="6" id="kpiCalIber" name="kpiCalIber" style="width:100%"></textarea>
				</dd>
			</dl>
			<dl class="group">
				<dt>指标解释：</dt>
				<dd>
					<textarea rows="6" id="kpiExplain" name="kpiExplain" style="width: 100%"></textarea>
				</dd>
			</dl>
	...
	<!--//comInbox-->
		<div class="centerBtnItem">
			<p><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="onAgree2()">保存</a></p>
		</div>
	</div>	
		...
		<div id="cc" class="easyui-layout" style="width:840px;height:450px;">   
		 	<div region="west" split="true" title="" style="width:220px;">
			 	<div class="easyui-tabs">  
		       		<div title="基础指标" id="base_kpi" style="overflow:auto;padding:10px;height: 395px;">
		       		</div> 
		       		<!-- <div title="复合指标" id="complex_kpi" style="overflow:auto;padding:10px;height: 395px;">
		       		</div>   -->
	 	  		</div> 
		 	</div>  
		 	<div region="center" title="" style="padding:5px;">
		 		<c:datagrid url="#" id="formTable" pagination="false" rownumbers="true" >
					<thead>
						<tr>
				...
				<div style="margin-top: 15px;">
					定义公式：<br/>
		 			<textarea id="kpi_formula" name="kpi_formula"  onselect="setCaret(this);" onclick="setCaret(this);" onkeyup="setCaret(this);" onfocus="setCaret(this);" style="width:600px;height:60px;font-size: 16px;margin-top: 10px;"></textarea>
		 		</div>
		 		<div style="margin-top: 15px;" class="oper_list">
					 运算符：&nbsp;&nbsp;<input id="ysf0" type="button" size="5" value="+" style="font-weight: bold;height: 30px;" onclick="javascript:doYsf('+');">&nbsp;
					 <input id="ysf1" type="button" size="5" value="-" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf('-');">&nbsp;
					 <input id="ysf2"  type="button" size="5" value="*" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf('*');">&nbsp;
					 <input id="ysf3"  type="button" size="5" value="/" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf('/');">
				 	 &nbsp;&nbsp;
					 <input id="ysf4"  type="button" size="5" value="(" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf('(');">&nbsp;
					 <input id="ysf5"  type="button" size="5" value=")" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf(')');">
					 <input id="ysf6"  type="button" size="5" value="指标" style="font-weight: bold; height: 30px;" onclick="javascript:doYsf('{kpi}');">
					 &nbsp;&nbsp;
					 <input id="ysf7"  type="button" size="5" value="清除" style="font-weight: bold; height: 30px;" onclick="javascript:doClare();">&nbsp;
					 <input id="ysf8"  type="button" size="15" value="锁定公式" style="font-weight: bold; height: 30px;" onclick="javascript:doLockFormula();">
				</div> 
				<div style="margin-top: 15px;">
					表达式：<br/>
    ...
    <div id="dim-dlg" class="easyui-dialog" title="维度" style="width:300px;height:380px;" data-options="closed:true,modal:true">
	    <div class="easyui-tabs">  
	   		<div title="基础指标" id="kpiDIV" style="overflow:auto;padding:10px;height: 285px;">
			</div> 
			<div title="基础维度" id="dimDIV" style="overflow:auto;padding:10px;height: 285px;">
			</div>
		</div>  
   	</div>
   	<div id="tjz-dlg" class="easyui-dialog" title="条件值" style="width:460px;height:330px;" data-options="closed:true,modal:true"  buttons="#dlg-buttons">
