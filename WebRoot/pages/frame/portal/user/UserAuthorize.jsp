...
<e:q4o var="curUser">
	select user_name from e_user where user_id=#userIdInput#
</e:q4o>
...
<script type="text/javascript">
	...
	$(function(){
			$('#authOrization').treegrid({
				...
				animate:true,
				collapsible:false,
				url:'<e:url value="/pages/frame/portal/user/UserAuthorizeAction.jsp"/>?eaction=getRoleTreeRootNode&userId=${param.userIdInput}',
				onBeforeExpand:function(node,param){  
                    $('#authOrization').treegrid('options').url = '<e:url value="/pages/frame/portal/user/UserAuthorizeAction.jsp"/>?eaction=getRoleTreeChildrenNode&userId=${param.userIdInput}&nodeId=' + node.RESOURCES_ID;
                }, 
	...
	function formatterR(value,rowData){
		...
		var userId = rowData.ROLE_CODE;
		var isChecked ="checked='checked'";
		if(value=='0'){
			isChecked="";
		}else if (rowData.FLAG=='ROLE') {
			isChecked += " disabled"
		}
	...
	function clickNodeCheck(id,type){
		...
		$.ajax({ 
		     type: "POST",
		     url: '<e:url value="/userTreeGridCheckNode.e"/>', 
		     data: {menuId:id, authType:type, userId:'${param.userIdInput}', checked:$("#C_"+id+"_"+type).attr("checked")},
		     dataType: "json",
...
<body >
	<div id="treedigridTools">
		<h2>用户授权 > 当前用户：${curUser.user_name}</h2>
		<div class="search-area">
			<p>
				<span class="easyui-font-red">注意：直接勾选将自动保存</span>
			
			<a href="javascript:void(0)" class="easyui-linkbutton easyui-linkbutton-gray" onclick="returnMenu()">返回</a>
			</p>
		</div>
	</div>
