...
<e:style value="/resources/themes/base/boncBase@links.css"/>
		...
		<script type="text/javascript">
			...
			function reflushDataGrid(){
				...
				$form.attr("action","<e:url value='/pages/frame/log/loginLogList.jsp' />");
				$form.submit();
			}
		</script>
	</head>
	...
	<e:if condition="${param.loginDate1!=null&&param.loginDate1!=''&&param.loginDate1!='null'}">
		...
		<div id="tb"  class="newSearchA">
				...
				<div class="search-area">
					...
					用户客户端IP: <input id="clientIp" style="width:100px" type="text" name="clientIp" value="${param.clientIp }" class="easyui-validatebox" validType="length[0,50]">
					<%-- 地市: <e:select id="area_no" name="area_no" items="${areaNoList.list}" label="area_desc" value="area_no" headLabel="全部" headValue="-1" defaultValue="${param.area_no }"/> --%>
					开始时间: <c:datebox id="loginDate" style="width:100px" name="loginDate" required="true" format="yyyymmdd" defaultValue='${loginDate_val}'/>
					结束时间: <c:datebox id="loginDate1" style="width:100px" name="loginDate1" required="true" format="yyyymmdd" defaultValue='${loginDate_val1}'/>
		...
		<c:datagrid url="/pages/frame/log/logAction.jsp?eaction=loginLog&loginId=${param.loginId }&area_no=${param.area_no }&clientIp=${param.clientIp }&loginDate=${loginDate_val}&loginDate1=${loginDate_val1}" id="loginLogListTable" nowrap="false" pageSize="${Number_val }" fit="true"  border="false" toolbar="#tb">
			<thead>
				<tr>
					...
					<th field="LOGIN_DATE" width="110" align="center" formatter="formatDAT_loginLogListTable">
						登录时间
					</th>
					<th field="LOGOUT_DATE" width="110" align="center" formatter="formatDAT_loginLogListTable">
						登出时间
					</th>
