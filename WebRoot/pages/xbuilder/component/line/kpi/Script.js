	...
	$("input[readonly='readonly']").droppable({
		...
		onDrop:function(e,source){
			...
			if($(source).attr("node-id")==undefined){//指标库选择
				selectLineKpiSuccess(lineKpiSelector.currentItem);
			}else{//计算列选择
				var itemObj = {
						id:"",
						column:"",
...
function validLineKpiRow(){
	...
	$("#line_tb2 tr:gt(0)").each(function(index){
		var ser = $(this).find(":text.line_ser").val();
 ...
 * 设置排序列
 */
function line_fun_SetOrd(){
	var info={};
	var componentId = StoreData.curComponentId;
	var containerId = StoreData.curContainerId;
	var isextcolumn = $("#line_ord").attr("isextcolumn");
	var kpiId = $("#line_ord").attr("kpiId");
	var kpiColumn = $("#line_ord").attr("kpiColumn");
	var extcolumnid = $("#line_ord").attr("extcolumnid");
 ...
 * 设置维度列
 */
function line_fun_SetDim(){
	var info={};
	var componentId = StoreData.curComponentId;
	var containerId = StoreData.curContainerId;
	var dimension = $("#line_dim").attr("kpiColumn");
	if(typeof(dimension)!="undefined"&&dimension!=""){
		info.dimFieldId = $("#line_dim").attr("kpiId");
...
function line_fun_SetKpi(){
	...
	$("#line_tb2 tr:gt(0)").each(function(index,item){
		var line_ser = $(this).find(":text.line_ser").attr("kpiId")==""?$(this).find(":text.line_ser").attr("extcolumnid"):$(this).find(":text.line_ser").attr("kpiId");
		if(kpiMap[line_ser]==undefined){
			kpiMap[line_ser]=line_ser;
		}else{
			hasRepeatKpi = true;
			$.messager.alert("线图","指标"+$(this).find(":text.line_ser").val()+"已添加，不能重复添加！","info");
			return false;
		}
	});
	if(hasRepeatKpi){
	...
	$("#line_tb2 tr:gt(0)").each(function(i){
		var kpiItem = {};
		var $serInput = $(this).find(":text.line_ser");
		...
		var yaxisid = $(this).find(":text.line_ref").combobox("getValue");
	...
	if(errMsg!=""){
		//top.$.messager.alert("线图",errMsg,"info");
		return;
	}
	$('#line_dialog2').dialog('close');
	cn.com.easy.xbuilder.service.component.LineService.setKpi(StoreData.xid,containerId,componentId,$.toJSON({kpiList:kpiArr}),function(data,exception){});
}
		...
		if(jsonObj!=null&&jsonObj!=undefined){
			...
			if(jsonObj.xaxis!=null&&jsonObj.xaxis!=undefined){
				...
				if(jsonObj.xaxis.dimFiledId!=""){
					  $("#line_dim").val(kpiColMap[jsonObj.xaxis.dimFiledId].kpidesc); 
					  $("#line_dim").attr("kpiId",jsonObj.xaxis.dimFiledId);
					  $("#line_dim").attr("kpiColumn",jsonObj.xaxis.dimfield); 
				}
			    $("#line_ord").val(kpiColMap[sortColId].kpidesc);
			    $("#line_ord").attr("kpiId",jsonObj.xaxis.sortFiledId);
			    $("#line_ord").attr("kpiColumn",jsonObj.xaxis.sortfield);
			    $("#line_ord").attr("isextcolumn",jsonObj.xaxis.sortExtField!=""?"true":"false");
			    $("#line_ord").attr("extcolumnid",jsonObj.xaxis.sortExtField);
				$("#line_sorttype").combobox("setValue",jsonObj.xaxis.sortType); 
			}
			...
			if(typeof(kpiList)!="undefined"&&kpiList!=null){
				...
				for(var i=0;i<kpiList.length;i++){
					...
					var $serInput=$row.find(":text.line_ser");
 ...
 * @param inputId
 */ 
function openLineKpiDialog(){
	var cubeId = StoreData.cubeId;
	lineKpiSelector.open(cubeId);
}
 ...
 * @param kpiArr
 */
function selectLineKpiSuccess(itemObj){
	var kpi = itemObj;
	var kpiType = kpi.kpiType;
	var curSelectInputId=$("#curSelectInput").val();
	if($("#"+curSelectInputId).attr("name")=="line_dim"){
		if(kpiType=='kpi'||kpiType=="extcolumn"){
			$.messager.alert("线图","指标不能作为维度使用！","info");
			return;
		}
	}if($("#"+curSelectInputId).attr("name")=="line_ser"){
		if(kpiType=='dim'||kpiType=='property'){
			$.messager.alert("线图",(kpiType=='dim'?"维度":"属性")+"不能作为指标使用！","info");
			return;
		}
	}
	...
	else if(curSelectInputId=="line_ord"){
		line_fun_SetOrd();
	}else if(curSelectInputId=="line_dim"){
		if(kpi.isextcolumn=="true"){
			$.messager.alert("线图","计算列不能作维度！","error");
		}else{
			line_fun_SetDim();
		}
	}
 ...
 * @returns {String}
 */
function getAllLineSelectedKpiIds(){
	var kpiIds = "";
	if($("#line_ord").attr("kpiId")!=undefined&&$("#line_ord").attr("kpiId")!=""){
		kpiIds+=$("#line_ord").attr("kpiId")+",";
	}
	if($("#line_ord").attr("extcolumnid")!=undefined&&$("#line_ord").attr("extcolumnid")!=""){
		kpiIds+=$("#line_ord").attr("extcolumnid")+",";
	}
	if($("#line_dim").attr("kpiId")!=undefined&&$("#line_dim").attr("kpiId")!=""){
		kpiIds+=$("#line_dim").attr("kpiId")+",";
	}
	$("#line_tb2 tr:gt(0)").each(function(index,item){
		var line_ser = $(this).find(":text.line_ser");
		if($(line_ser).attr("kpiId")!=undefined&&$(line_ser).attr("kpiId")!=""){
			kpiIds+=$(line_ser).attr("kpiId")+",";
		}
		if($(line_ser).attr("extcolumnid")!=undefined&&$(line_ser).attr("extcolumnid")!=""){
			kpiIds+=$(line_ser).attr("extcolumnid")+",";
		}
	});
...
function line_removeKpiRow(){
	...
	if(obj[0]==undefined){
		...
		cn.com.easy.xbuilder.service.XComponentService.removeKpiRow(info,function(data,exception){
			 if(exception!=undefined){
				 $.messager.alert("线图","删除指标发生错误："+exception,"info");
			 }else{
				 $(obj).remove();
			 }
		});
...
function lineUpdataDatasetUI(action){
	...
	if(action=="edit"){
		if(lineCaculateColumn.getColumnOldName()==$("#line_ord").val()){
			$("#line_ord").val(lineCaculateColumn.currentEditColumnObj.name);
			info.kpistoreColId=$("#line_ord").attr("extcolumnid");
			info.kpiColumn=$("#line_ord").attr("extcolumnid");
			info.kpiDesc=$("#line_ord").val();
			cn.com.easy.xbuilder.service.XComponentService.addKpiStoreCol(info,function(data,exception){
				if(exception!=undefined){
					$.messager.alert("线图","保存指标时发生错误："+exception,"info");
				}
		...
		$("#line_tb2 tr:gt(0)").each(function(index,item){
			if(lineCaculateColumn.getColumnOldName()==$(this).find(":text.line_ser").val()){
				$(this).find(":text.line_ser").val(lineCaculateColumn.currentEditColumnObj.name);
				$(this).find("input[name='line_sername']").val(lineCaculateColumn.currentEditColumnObj.name);
				info.kpistoreColId=$(this).find(":text.line_ser").attr("extcolumnid");
				info.kpiColumn=$(this).find(":text.line_ser").attr("extcolumnid");
				info.kpiDesc=$(this).find(":text.line_ser").val();
				cn.com.easy.xbuilder.service.XComponentService.addKpiStoreCol(info,function(data,exception){
						if(exception!=undefined){
							$.messager.alert("线图","保存指标时发生错误："+exception,"info");
						}else{
		...
		var componentId = StoreData.curComponentId;
		var containerId = StoreData.curContainerId;
		if(lineCaculateColumn.currentEditColumnObj.name==$("#line_ord").val()){
				var sortType = $("#line_sorttype").combobox("getValue");
				var kpiType = $("#line_ord").attr("kpiType");
				var info={ord:"",isextcolumn:"true",kpiType:kpiType,sortType:sortType};
				cn.com.easy.xbuilder.service.component.LineService.setSortField(StoreData.xid,containerId,componentId,$.toJSON(info),function(data,exception){
					$("#line_ord").val("");
					$("#line_ord").attr("kpiId","");
					$("#line_ord").attr("kpiColumn","");
					$("#line_ord").attr("isextcolumn","false");
					$("#line_ord").attr("extcolumnid","");
					//调用删除kpistore方法
					alert($.toJSON(lineCaculateColumn.currentEditColumnObj));
					cn.com.easy.xbuilder.service.component.LineService.deleteKpiStoreCol(StoreData.xid,lineCaculateColumn.currentEditColumnObj.id,function(data,exception){});
				});
		}
		
		$("#line_tb2 tr:gt(0)").each(function(index,item){
				if(lineCaculateColumn.currentEditColumnObj.name==$(this).find(":text.line_ser").val()){
