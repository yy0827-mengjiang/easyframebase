		...
		<script type="text/javascript">
			...
			function reportCZ(value,rowData){
				...
				*/
				...
				if(parseInt(state)==4){
					var edit = '<a href="javascript:void(0);" class="moreBianji" title="编辑" style="margin:0 5px;" onclick="updReport(\''+rowData.REPORT_ID+'\',\''+state+'\')">编辑</a>';
					if(userId != rowData.CREATE_USER){
					...
					res = upd+pub+file+authorization+more;
				}
				return  res;
			}
			...
			function showNotStopInfo(){
				$.messager.alert("提示信息","<br/>未上架，不允许下架！",'info');
			}
			...
			function updReport(reportId,state){
				...
				//var lw = screen.width-10;
				//var sourceDataLw=$.ajax({url: appBase+'/getDLWidth.e?xid='+reportId,type:"POST",cache: false,async: false}).responseText;
				//if(sourceDataLw == lw){
				//	window.open(appBase+'/xLiEdit.e?xid='+reportId);//修改在新窗口中打开
				//	window.returnValue=false;
				//}else{
				//	$.messager.confirm('确认信息', '您的分辨率宽为['+(screen.width+10)+']和已保存宽为['+(parseInt(sourceDataLw)+10)+']的不一样，建议使用相同的分辨率进行编辑。如果您继续布局的显示可能会有问题，需要您重新调整，是否继续？', function(r){
				//		if(r){
				//			window.open(appBase+'/xLiEdit.e?xid='+reportId);//修改在新窗口中打开
				//			window.returnValue=false;
				//		}
			...
			function dblClick(node){
				...
				if($("#cur_name").val()=='')
					return;
				$.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=CURREPORT",{resources_name:res_name,parent_id:node.id,report_id:rep_id},function(data){
					if(parseInt($.trim(data))==0){
						$.messager.confirm('确认信息','确定要上架到当前菜单下?',function(r){  
						    if (r){  
						        $.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp",info,function(data){
									if(parseInt($.trim(data))>0){
			...
			function delReport(id){
				$.messager.confirm('确认信息','删除报表将删除报表的相关信息，确定删除吗?',function(r){  
					if(r){
						$.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=DELETEALLRP",{report_id:id},function(data){
							if(parseInt($.trim(data))>0){
			...
			$(function(){
				$('#reportDescForm').form({  
					url:appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=INSERTREPORT",
				    onSubmit: function(){  
				    ...
				    success:function(data){  
						...
						if(temp>0) {
							...
							$.messager.alert("信息","报表描述保存过程中出现错误，请联系管理员！","error");;
						}
					}
				});
			...
			function desReport(Id,reportName){
				...
				var postUrl=appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp";
			...
			$(function(){
				...
				$('#fileForm').form({
					url:appBase+'/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=addFile',
					success:function(data){
			...
			function deleteFile(filePath){
				$.messager.confirm('确认信息','确定删除?',function(r){  
				    if (r){  
				        $.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=deleteFile",{filePath:filePath},function(data){
							if(data==1){
								var info = {};
								info.reportId = CURRENT_REPORT_ID;
								//$("#fileListTable").datagrid("options").queryParams=info;
			...
			function stopUseReport(id){
		  		$.messager.confirm('确认信息','下架报表后将无法访问，只有重新上架后才可以访问，确定下架吗?',function(r){  
					if(r){
						$.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=stopUseReport",{report_id:id},function(data){
							if(parseInt($.trim(data))>0){
								$("#reportTable").datagrid("load",$("#reportTable").datagrid("options").queryParams);
		  	...
		  	function examin(reportId,reportName,user,reportstate){
		  		...
		  		info.user = user;
		  		$.post(appBase+"/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=checkexamin",info,function(data){
					if($.trim(data) == 'N'){
	...
	<body>
		<div id="tbar" class="unBorder">
			...
			<div class="search-area">
				...
				状态  ： <e:select items="${state.list}" label="code_desc" value="code" name="state" id="state" class="easyui-validatebox" required="true" />
				创建时间： <c:datebox id="create_date" style="width:100px" name="create_date" required="false" format="yyyymmdd"/>
<!--				修改时间： <input class="easyui-datebox" style="width: 100px" id="update_date" name="update_date">-->
<!--				附加信息： <input type="text" style="width: 100px" id="menu_name" name="menu_name">-->
				...
				<!-- 
					addReport():选择数据方式后弹出相应设计界面
					toSql() ：弹出数据方式为自定义sql的设计界面（上海用到）
				-->
			</div>
		</div>
		<c:datagrid
			url="pages/xbuilder/pagemanager/ReportAction.jsp?eaction=LIST" fit="true" data-options="border:false" remoteSort="true" 
			id="reportTable" pageSize="15" style="width:auto;" onLoadSuccess="showMore" onDblClickRow="toShowReport"
...
<!--					<th field="UPDATOR_NAME" width="60" align="center" sortable="true">-->
<!--						修改人-->
<!--					</th>-->
<!--					<th field="UPDATE_DATE" width="110" align="center" sortable="true">-->
<!--						修改时间-->
<!--					</th>-->
...
<!--					<th field="TOTAL" width="50" align="center" formatter='sendDetail' sortable="true">-->
<!--						上架数-->
<!--					</th>-->
		...
		<div class="easyui-dialog" id="report_tree_dialog"
			title="上架到菜单(双击菜单上架)"
			data-options="closed:true,top:90,modal:true,onClose:closeDialog"
			...
			<a:tree id="report_tree"
				url="pages/xbuilder/pagemanager/ReportAction.jsp?eaction=RTREE"
			...
			<form action="" method="post" id="reportDescForm">
				<table width="100%" class="windowsTable">
					...
					<tr>
						...
						<td>
							<textarea style="width: 91%; height: 100px;" id="report_desc"
		...
		<div id="userInfoDialog" title="用户信息">
			<form action="" method="post" id="userInfoForm">
				...
				<c:datagrid
					url="/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=USERLIST"
			...
			<form id="fileForm" name="fileForm" method="post" enctype="multipart/form-data">
				<div class="contents-head">
					...
					<div class="search-area">
						...
						<input type="file" id="uploadFile" name="uploadFile" size="280" style="width: 280px;">
						<a href="javascript:void(0)" onclick="subFileUpload()" class="easyui-linkbutton" >提交</a>
					</div>
				</div>
			</form>
			<c:datagrid id="fileListTable" url="/pages/xbuilder/pagemanager/ReportAction.jsp?eaction=fileList" pageSize="100" style="height:200px;"  nowrap="false" pagination="false">
				<thead>
					<tr>
						<th field="FILE_NAME" width="120" align="center">
							文档名称
						</th>
						<th field="CREATE_TIME" width="40" align="center">
							上传时间
						</th>
						<th field="USER_ID" width="40" align="center">
							上传人
						</th>
						<th field="opt" width="60" align="center" formatter="formatOpt">
							操作
						</th>
					</tr>
				</thead>
		...
		<div id="depTreeDialog" title="局方部门">
			<a:tree id="depTree"
				url="pages/xbuilder/pagemanager/ReportAction.jsp?eaction=DEPTREE"
				onSelect="slectDep" />
		</div>
		<!-- 审核 -->
