...
<e:description>某地区型号3的图片URL </e:description>
<e:q4o var="hjOnexlinImg3">
			SELECT  DISTINCT
				    Y."IMG_URL"
				FROM "DM_M_TERM_USER_CHANGE" X,"DIM_IMG_NO" Y
				WHERE  1=1
...
<e:description> 某地区型号2的换入销量Top3  </e:description>
<e:q4l var="hjOnein2">
			SELECT
				  X."TERMINAL_NUM" a,Y."CITY_NAME" b
				FROM "DM_M_TERM_USER_CHANGE" X,"DIM_CITY_NO" Y
...
<e:description> 某地区型号2的迁出销量Top3  </e:description>
<e:q4l var="hjOneout2">
			SELECT
				  X."TERMINAL_NUM" a,Y."CITY_NAME" b
				FROM "DM_M_TERM_USER_CHANGE" X,"DIM_CITY_NO" Y
...
<e:description>某地区型号3的换入总销量 </e:description>
<e:q4o var="hjOnexlinSum3">
			SELECT
				  "sum"(X."TERMINAL_NUM") xlSumin
				FROM "DM_M_TERM_USER_CHANGE" X
				WHERE  1=1	
				...
				and X."IN_BRAND"='${param.br3}'
				and X."IN_MODEL"='${param.brxh3}'										 
</e:q4o>
<e:description> 某地区型号3的换入销量Top3  </e:description>
<e:q4l var="hjOnein3">
			SELECT
				  X."TERMINAL_NUM" a,Y."CITY_NAME" b
				FROM "DM_M_TERM_USER_CHANGE" X,"DIM_CITY_NO" Y
...
<e:description> 某地区型号3的迁出总销量  </e:description>
<e:q4o var="hjOnexloutSum3">
			SELECT
				  "sum"(X."TERMINAL_NUM") xlSumout
				FROM "DM_M_TERM_USER_CHANGE" X
				WHERE  1=1	
				...
				and X."OUT_BRAND"='${param.br3}'
				and X."OUT_MODEL"='${param.brxh3}'									 
</e:q4o>
<e:description> 某地区型号3的迁出销量Top3  </e:description>
<e:q4l var="hjOneout3">
			SELECT
				  X."TERMINAL_NUM" a,Y."CITY_NAME" b
				FROM "DM_M_TERM_USER_CHANGE" X,"DIM_CITY_NO" Y
...
<e:description> 某地区型号的(换出的)换机周期 </e:description>
<e:q4l var="hjTwozq">
			SELECT DISTINCT
				...
				and X."BRAND_NO" IN ('${param.br1}','${param.br2}','${param.br3}')
				and X."MODEL_NO" IN ('${param.brxh1}','${param.brxh2}','${param.brxh3}')									 
</e:q4l>
...
<e:description> 某地区某型号3的流出sum </e:description>
<e:q4o var="hjThreeoutSum3">
			SELECT
					"sum" (Y."TERMINAL_NUM") lcSumin
				FROM
					(SELECT 	
...
<e:description> 某地区品牌3的流出Top5</e:description>
<e:q4l var="hjThreeout3">
			SELECT
				Z."MODEL_NAME" b,
				Y."TERMINAL_NUM" a
			FROM
				...
				and X."BRAND_NO"='${param.br3}'
				and X."MODEL_NO"='${param.brxh3}') Y,"TRMNL_DEVINFO" Z
			WHERE 1=1
			AND Y."OUT_MODEL"=Z."MODEL_NO"
			ORDER BY   Y."TERMINAL_NUM"  DESC
			LIMIT 5													 
</e:q4l>
<e:description> 某地区型号3的流入sum </e:description>
<e:q4o var="hjThreeinSum3">
			SELECT
					"sum" (Y."TERMINAL_NUM") lrSumin
				FROM
					(SELECT 	
...
<e:description> 某地区品牌3的流入Top5</e:description>
<e:q4l var="hjThreein3">
			SELECT
				Z."MODEL_NAME" b,
				Y."TERMINAL_NUM" a
			FROM
...
<e:description> 换机区域对比图2前10的总和</e:description>
<e:q4o var="npie22">
SELECT
	"sum"(Y."TERMINAL_NUM") A
FROM
...
<e:description> 换机区域对比图3</e:description>
<e:q4l var="npie3">
	SELECT
				Z."CITY_NAME",
				Y."TERMINAL_NUM" a
...
<e:description> 换机区域对比图3前10的总和</e:description>
<e:q4o var="npie33">
SELECT
	"sum"(Y."TERMINAL_NUM") A
FROM
...
<SCRIPT>
	$(function(){
		...
		//迁入in	3	
		<e:forEach items="${hjOnein3.list}" var="item" indexName="index">
			<e:if condition="${item.a != null && item.a != ''}">
				var inVal91span =(Math.round(Number('${item.a}') / Number('${hjOnexlinSum3.xlSumin}') * 10000) / 100.00 + "%");
				$("#s9"+${index}).html(inVal91span); 				
			</e:if>
		</e:forEach>
		
		//迁出out 3
		<e:forEach items="${hjOneout3.list}" var="item" indexName="index">
			<e:if condition="${item.a != null && item.a != ''}">
				var inVal101span =(Math.round(Number('${item.a}') / Number('${hjOnexloutSum3.xlSumout}') * 10000) / 100.00 + "%");
				$("#s10"+${index}).html(inVal101span); 				
			</e:if>
		</e:forEach>
		
		//流出 2
		<e:forEach items="${hjThreeout3.list}" var="item" indexName="index">
			<e:if condition="${item.a != null && item.a != ''}">
				var inVal111span =(Math.round(Number('${item.a}') / Number('${hjThreeoutSum3.lcSumin}') * 10000) / 100.00 + "%");
				$("#s11"+${index}).html(inVal111span); 				
			</e:if>
		</e:forEach>
		
		//流入 2
		<e:forEach items="${hjThreein3.list}" var="item" indexName="index">
			<e:if condition="${item.a != null && item.a != ''}">
				var inVal121span =(Math.round(Number('${item.a}') / Number('${hjThreeinSum3.lrSumin}') * 10000) / 100.00 + "%");
				$("#s12"+${index}).html(inVal121span); 				
			</e:if>
		</e:forEach>
	});
  ...
  //饼图2数据
  	$(function(){
  	 ...
  	 //饼图3数据
  	$(function(){
  		var brType03='${param.brname3}'+"-"+'${param.brxhname3}';//品牌-型号
  		var num5= Number('${hjThreeoutSum3.lcSumin }');//该型号3的流出总量
  		var num6= Number('${npie33.a }');//该型号3Top10的总量
  		var qtzb2 = num5-num6+"";//型号3其它
  		var x1=[];//地市1	  		
  		var ser=[];//数据1
  		<e:if condition="${e:length(npie3.list)>0}" var="check1">
  			if(num5 == num6){
  				<e:forEach items="${npie3.list}" var="item" indexName="index">			
			  		x1.push('${item.CITY_NAME}');	
			  		ser.push({
							value: '${item.a}',
			                name: '${item.CITY_NAME}'	               
				...
				var siteChart1 = echarts.init(document.getElementById('pie3Chart'));
				chartpic1(brType03,x1,ser,siteChart1);
  			}else{
  				<e:forEach items="${npie3.list}" var="item" indexName="index">			
		  		x1.push('${item.CITY_NAME}');	
		  		ser.push({
    	...
    	<div>
					<div class="AdminProduct phonechange">
						<div class="halfchange">
							...
							<!-- //品牌2 -->
							<!-- 品牌3 -->
							<e:if condition="${e:length(hjOnein3.list)>0 || e:length(hjOneout3.list)>0}" var="check10"> 
							<div class="placebox">
								<ul class="UlCompare">
									<e:forEach items="${hjOnein3.list}" var="item" indexName="index">
										<li>
											<span>${index+1 }</span>
											<span>${item.b}</span>
											<span  id ="s9${index}" class="red"></span>
										</li>
									</e:forEach>
								</ul>
								<p class="arrow01"></p>
								<e:if condition="${param.city_no ne 'qxz'}" var="check115">
									 	<div class="city">	${param.city_name } </div>
									</e:if>
									<e:else condition="${check115 }">
								 		<div class="city"> ${param.pronameH}</div>
								 	</e:else>	
								<div class="app"><img src='<e:url value="${hjOnexlinImg3.IMG_URL}"/>'></div>
								<e:if condition="${param.city_no ne 'qxz'}" var="check116">
									 	<div class="city">	${param.city_name } </div>
									</e:if>
									<e:else condition="${check116 }">
								 		<div class="city"> ${param.pronameH}</div>
								 	</e:else>	
								<p class="arrow02"></p>
								<ul class="UlCompare">
									<e:forEach items="${hjOneout3.list}" var="item" indexName="index">
										<li>
											<span>${index+1 }</span>
											<span>${item.b}</span>
											<span  id ="s10${index}" class="red"></span>
										</li>
									</e:forEach>
								</ul>
							</div>
							...
							<div style="width:100%;text-align:center;">${param.brname3}-${param.brxhname3}</div>														
							<!-- //品牌3 -->
						</div>
						<div class="halfchange">
							<h2 class="time">换机周期</h2>
							<!-- 换机周期echarts预留位置 -->
							<div style="margin:65px  0 0 0;width:95%;">
							<e:if condition="${e:length(hjTwozq.list)>0}" var="check3">														
								<c:nbar id="picbar1" height="400"  width="550" 
					...
					<div class="AdminProduct">
						...
						<div class="fldiv">
							...
							<div style="width:100%;height:45%;margin: 34px 0 34px 0">
						...
						<!-- //品牌1 -->
						<!-- 品牌2 -->
						<div class="fldiv"  style="width:29%; margin-left: 15px;">
							<div style="width:100%;height:45%;">
								<div class="loutright" style="width:100%">
									<div class="lediv">
						...
						<div class="fldiv" style="width:29%;margin-left: 15px;">
							<div style="width:100%;height:45%;">
								<div class="loutright" style="width:100%">
									<div class="lediv">
										<img src='<e:url value="${hjOnexlinImg3.IMG_URL}"/>'>
										<p>${param.brname3}-${param.brxhname3}</p>
										<p style="color: red;">(${hjThreeoutSum3.lcSumin })</p>
									</div>
									<div class="arrow01-compare"></div>
									<div class="tabdiv">
										<table cellpadding="0" cellspacing="0" class="TableMarket normal">
											...
											<tbody>
											 	<e:if condition="${e:length(hjThreeout3.list)>0}" var="check11">
											 
											 	<e:forEach items="${hjThreeout3.list}" var="item" indexName="index">											
													<tr>
														<th><span class="thead0${index+1 }">${index+1 }</span></th>
														<td>${item.b}</td>
														<td><span id="s11${index}"></span></td>
													</tr>
												</e:forEach>
												</e:if>
							...
							<div style="width:100%;height:45%;margin: 34px 0 34px 0">
								<div class="loutright" style="width:100%">
									<div class="lediv">
										<img src='<e:url value="${hjOnexlinImg3.IMG_URL}"/>'>
										<p>${param.brname3}-${param.brxhname3}</p>
										<p style="color: red;">(${hjThreeinSum3.lrSumin })</p>
									</div>
									<div class="arrow02-compare"></div>
									<div class="tabdiv">
										<table cellpadding="0" cellspacing="0" class="TableMarket normal">
											...
											<tbody>
											<e:if condition="${e:length(hjThreein3.list)>0}" var="check12">
											
											   <e:forEach items="${hjThreein3.list}" var="item" indexName="index">											
													<tr>
														<th><span class="thead0${index+1 }">${index+1 }</span></th>
														<td>${item.b}</td>
														<td><span id="s12${index}"></span></td>
													</tr>
												</e:forEach>
											</e:if>
					...
					<div class="AdminProduct">
						...
						<div id="pie1Chart" class="chartAdmin" style="width:370px;height:400px"></div>
						<div id="pie2Chart" class="chartAdmin" style="width:370px;height:400px"></div>
						<div id="pie3Chart" class="chartAdmin" style="width:370px;height:400px"></div>
							<!-- 品牌1 -->		
							...
							<!-- 品牌3 -->
							<div class="chartAdmin">
								<e:if condition="${e:length(npie3.list)>0}" var="check13">
								<c:npie  id='pie3' 
									 width='350'
								 	 height='350'
								 	  title="${param.brname3 }-${param.brxhname3 }"
									 dataLabels="false"	
									 colors="['#01d6ff','#ff8500','#ffcf13','#86d438','#8945fe','#01d6ff','#ff8500','#ffcf13','#86d438','#8945fe'] "							
									 items="${npie3.list}"
									 a="占比"
