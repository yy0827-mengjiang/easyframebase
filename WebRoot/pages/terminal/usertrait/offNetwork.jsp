...
<e:q4l var="age">
	...
	WHERE 
		A."AGE_NO" = B."AGE_NO" AND "LINE_TYPE" = '3'
...
<e:q4l var="meal">
	...
	WHERE
		A."MEL_NO" = B."MEAL_NO" AND A."LINE_TYPE" = '3' AND B."IS_MEAL" = '0'
		AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
		<e:if condition="${area_no != 'qg' }">
			AND A."AREA_NO" = '${area_no }'
...
<e:q4l var="arup">
	...
	WHERE 
		A."LINE_TYPE" = '3'	AND A."ARPU_NO" = B."ARPU_NO"
...
<e:q4l var="content">
	...
	FROM
		(SELECT 
				...
				WHERE 
				A."CONTENT_NO" = B."CONTENT_NO" AND A."LINE_TYPE" = '3'
				AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
				<e:if condition="${area_no != 'qg' }">
					AND A."AREA_NO" = '${area_no }'
...
<e:q4l var="loyalty">
	...
	WHERE
		A."LOYALTY_NO" = B."LOYALTY_NO" AND A."LINE_TYPE" = '3'
		AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
		<e:if condition="${area_no != 'qg' }">
			AND A."AREA_NO" = '${area_no }'
...
<!--EasyUI1.5 Css层叠样式表 -->
	...
	//查询按钮
 	function doSearch(){   	
		...
		if(parseInt(acct_month) < parseInt(acct_monthbefore)){
			...
			window.location.href = '<e:url value="/pages/terminal/usertrait/offNetwork.jsp"/>?area=' + area + '&city=' + city + '&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month;
   			}
		}
    </script>
	...
	//总人数饼图男
 	$(function(){
		var objdata =jQuery.parseJSON('${e:java2json(totalB3.list)}');
		...
		if(objdata.length > 0){
			if(objdata.length == 1){
				...
				newBoy3(legendata,objdata,colors);
			}else{
				colors.push('#4F94CD');
				colors.push('#DEDEDE');
 	...
 	function newBoy3(legendata,seriesdata,colors){
	...
	//总人数饼图女
 	$(function(){
		var objdata =jQuery.parseJSON('${e:java2json(totalG3.list)}');
		...
		if(objdata.length > 0){
			if(objdata.length == 1){
				...
				newGirl3(legendatanv,objdata,colors);
			}else{
				colors.push('#FF6EB4');
				colors.push('#DEDEDE');
				for(var i=0;i<objdata.length;i++){
 	...
 	function newGirl3(legendatanv,seriesdata,colors){
       // 基于准备好的dom，初始化echarts实例
	...
	//流量占收比面积图
 	$(function(){
 		var objdata3 =jQuery.parseJSON('${e:java2json(liul3.list)}');
		var legendatall3 = new Array();
		var datas3 = new Array();
		if(objdata3.length > 0){
			for(var i=0;i<objdata3.length;i++){
				legendatall3.push(objdata3[i].a);
				datas3.push(objdata3[i].b);
			}
			ll1(legendatall3,datas3);
		}else{
			$("#charts3").html("暂无数据！");
		}
 	...
 	function ll1(legendatall3,datas3){
       // 基于准备好的dom，初始化echarts实例
	...
	<div title="离网" selected="true">
		...
		<div class="userarea">
			<div class="userbox" style="width: 33%;">
				...
				<!-- charts图表预留位置 -->
				<e:if condition="${e:length(age.list)<=0}" var="select13">
					暂无数据！
				</e:if>
				<e:else condition="${select13 }">
					<c:npie id="pie1" distance="1" percentage="false"
			...
			<div class="userbox" style="width: 33%;">
				<h2>套餐TOP5</h2>
				<!-- charts图表预留位置 -->
				<e:if condition="${e:length(meal.list)<=0}" var="select23">
					暂无数据！
				</e:if>
				<e:else condition="${select23 }">
			...
			<div class="userbox" style="width: auto;">
				<h2>ARPU</h2>
				<!-- charts图表预留位置 -->
				<e:if condition="${e:length(arup.list)<=0}" var="select33">
					暂无数据！
				</e:if>
				<e:else condition="${select33 }">
		...
		<div class="userarea">
			<div class="userbox" style="width: 33%;">
				<h2>内容TOP5</h2>
				<!-- 排行 -->
				<e:if condition="${e:length(content.list)<=0}" var="select43">
					暂无数据！
				</e:if>
				<e:else condition="${select43 }">
					<ul class="ulrank">						
			...
			<div class="userbox" style="width: 33%;height:259px;">	
	            <h2 class="total">总人数<strong>${total3.a }</strong></h2>	
	            <div id="tt3" style="width: 100%">							
	                ...
	                <div id="myStat"></div>
	                 ...
	                 <p style=" position: relative;z-index: 300; bottom: 86px; width: 97%; text-align: center;font-size: 19px;color: #4F94CD;">
	                 	<e:if condition="${totalB33.b == '' || totalB33.b eq null }" var="selectif1">
	                		0
	                	</e:if>
	                 ...
	                 <p style="position: relative;z-index: 300; bottom: 49px; width: 97%; text-align: center;font-size: 21px;color: #999;">
	                 	<e:if condition="${totalB33.c == '' || totalB33.c eq null }" var="selectif2">
	                		0%
	                	</e:if>
		              ...
		              <div id="myStat1"></div>	             
		               <div class="myimg"></div>
		                <p style=" position: relative;z-index: 300; bottom: 86px; width: 97%; text-align: center;font-size: 19px;color: #FF6EB4;">
		                	<e:if condition="${totalG33.d == '' || totalG33.d eq null }" var="selectif3">
		                		0
		                	</e:if>
	                 	...
	                 	<p style="position: relative;z-index: 300; bottom: 49px; width: 97%; text-align: center;font-size: 21px;color: #999;">
		                 	<e:if condition="${totalG33.e == '' || totalG33.e eq null }" var="selectif4">
		                		0%
		                	</e:if>
			...
			<div class="userbox" style="width: 33%;">
				<h2>应用TOP5</h2>
				<!-- 排行 -->
				<e:if condition="${e:length(app.list)<=0}" var="select63">
					暂无数据！
				</e:if>
				<e:else condition="${select63 }">
					<ul class="ulrank">
		...
		<div class="userarea">
			<div class="userbox" style="width: 33%;">
				<h2>品牌忠诚度</h2>
				<!-- charts图表预留位置 -->
				<e:if condition="${e:length(loyalty.list)<=0}" var="select73">
					暂无数据！
				</e:if>
				<e:else condition="${select73 }">
					<div class="charts">
			...
			<div class="userbox" style="width: 33%;">
				<h2>流量占收比</h2>
				<!-- charts图表预留位置 -->	
	    		<div class="charts" width="450" height="200" id="charts3"></div>	
				<!-- //charts图表预留位置 -->
			</div>
		</div>
		<!-- //用户特征离网 -->
