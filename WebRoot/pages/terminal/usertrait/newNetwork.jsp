...
<%-- <e:if condition="${param.startDay != '' && param.startDay != null }">
	<e:set var="start_month">${param.startDay }</e:set>
</e:if>
<e:set var="stop_month">${e:getDate('yyyyMM')}</e:set>
<e:if condition="${param.stopDay != '' && param.stopDay!=null }">
	<e:set var="stop_month">${param.stopDay }</e:set>
</e:if> --%>
...
<e:description>1.年龄</e:description>
<e:q4l var="age">
	SELECT
		B."AGE_NAME" a,
	...
	WHERE 
		A."AGE_NO" = B."AGE_NO" AND "LINE_TYPE" = '1'
...
<e:q4l var="meal">
	...
	WHERE
		A."MEL_NO" = B."MEAL_NO" AND A."LINE_TYPE" = '1' AND B."IS_MEAL" = '0'
		AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
		<e:if condition="${area_no != 'qg' }">
			AND A."AREA_NO" = '${area_no }'
...
<e:q4l var="arup">
	...
	WHERE 
		A."LINE_TYPE" = '1'	AND A."ARPU_NO" = B."ARPU_NO"
...
<e:q4l var="content">
	...
	FROM
		(SELECT 
				...
				WHERE 
				A."CONTENT_NO" = B."CONTENT_NO" AND A."LINE_TYPE" = '1'
				AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
				<e:if condition="${area_no != 'qg' }">
					AND A."AREA_NO" = '${area_no }'
...
<e:q4l var="loyalty">
	...
	WHERE
		A."LOYALTY_NO" = B."LOYALTY_NO" AND A."LINE_TYPE" = '1'
		AND A."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
						   AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
		<e:if condition="${area_no != 'qg' }">
			AND A."AREA_NO" = '${area_no }'
...
<!--EasyUI1.5 Css层叠样式表 -->
	...
	//查询按钮
 	function doSearch(){   	
		...
		alert("开始时间大于结束时间，请重新输入！");
  				...
  				window.location.href = '<e:url value="/pages/terminal/usertrait/newNetwork.jsp"/>?area=' + area + '&city=' + city + '&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month;
  			}	
		}
    </script>
	...
	<script type="text/javascript">	
 	$(function(){	
			var objdata =jQuery.parseJSON('${e:java2json(totalB1.list)}');
			...
			if(objdata.length > 0){
				if(objdata.length == 1){
					...
					newBoy(legendata,objdata,colors);
				}else{
					colors.push('#4F94CD');
					colors.push('#DEDEDE');
 	...
 	function newBoy(legendata,seriesdata,colors){
	...
	//总人数饼图女
	$(function(){	
		var objdata =jQuery.parseJSON('${e:java2json(totalG1.list)}');
		...
		if(objdata.length > 0){
			if(objdata.length == 1){
				...
				newGirl(legendatanv,objdata,colors);
			}else{
				colors.push('#FF6EB4');
				colors.push('#DEDEDE');
				for(var i=0;i<objdata.length;i++){
 	...
 	function newGirl(legendatanv,seriesdata,colors){
       // 基于准备好的dom，初始化echarts实例
	...
	//流量占收比面积图
 	$(function(){
 		var objdata =jQuery.parseJSON('${e:java2json(liul1.list)}');
		var legendatall1 = new Array();
		var datas1 = new Array();
		if(objdata.length > 0){
			for(var i=0;i<objdata.length;i++){
				legendatall1.push(objdata[i].a);
				datas1.push(objdata[i].b);
			}
			ll1(legendatall1,datas1);
		}else{
			$("#charts1").html("暂无数据！");
		}
 	...
 	function ll1(legendatall1,datas1){
       // 基于准备好的dom，初始化echarts实例
...
<div id="boncEntry">
	...
	<div  class="userarea">
		<div class="userbox" style="width: 33%;">
			...
			<!-- charts图表预留位置 -->
			<e:if condition="${e:length(age.list)<=0}" var="select1">
				暂无数据！
			</e:if>
		...
		<div class="userbox" style="width: 33%;">
			...
			<!-- charts图表预留位置 -->
			<div>
				<e:if condition="${e:length(meal.list)<=0}" var="select2">
					暂无数据！
				</e:if>
		...
		<div class="userbox" style="width: auto;">
			<h2>ARPU</h2>
			<!-- charts图表预留位置 -->
			<e:if condition="${e:length(arup.list)<=0}" var="select3">
				暂无数据！
			</e:if>
			<e:else condition="${select3 }">
				<c:ncolumnline id="arup" width="450" height="200"
			        yaxis="title:,unit:"
	...
	<div class="userarea">
		...
		<h2>内容TOP5</h2>
			<!-- 排行 -->
			<e:if condition="${e:length(content.list)<=0}" var="select4">
				暂无数据！
			</e:if>
			<e:else condition="${select4 }">
			<ul class="ulrank">						
		...
		<div class="userbox" style="width: 33%;height:259px;">
			<h2 class="total">总人数<strong>${total.a }</strong></h2>
			<div id="tt1" style="width: 100%">
	             <div style="width:400px;height:200px;margin-left: 25px;">           
		            <div style="width:50%;float: left;margin-top: -43px;">
	                   <div id="myStat"></div>
	                   <div class="mgimg"></div>
	                 <p style=" position: relative;z-index: 300; bottom: 86px; width: 97%; text-align: center;font-size: 19px;color: #4F94CD;">${totalB.b }</p>
	                 <p style="position: relative;z-index: 300; bottom: 49px; width: 97%; text-align: center;font-size: 21px;color: #999;">${totalB.c }</p>
	              </div>         
		              ...
		              <div id="myStat1"></div>	             
		               <div class="myimg"></div>
		                 <p style=" position: relative;z-index: 300; bottom: 86px; width: 97%; text-align: center;font-size: 19px;color: #FF6EB4;">
		                	<e:if condition="${totalG.d == '' || totalG.d eq null }" var="selectif3">
		                		0
		                	</e:if>
	                 	...
	                 	<p style="position: relative;z-index: 300; bottom: 49px; width: 97%; text-align: center;font-size: 21px;color: #999;">
		                 	<e:if condition="${totalG.e == '' || totalG.e eq null }" var="selectif4">
		                		0%
		                	</e:if>
		...
		<div class="userbox" style="width: 33%;">
			<h2>应用TOP5</h2>
			<!-- 排行 -->
			<e:if condition="${e:length(app.list)<=0}" var="select6">
				暂无数据！
			</e:if>
			<e:else condition="${select6 }">
				<ul class="ulrank">
	...
	<div class="userarea">
		<div class="userbox" style="width: 33%;">
			<h2>品牌忠诚度</h2>
			<!-- charts图表预留位置 -->
			<e:if condition="${e:length(loyalty.list)<=0}" var="select7">
				暂无数据！
			</e:if>
			<e:else condition="${select7 }">
				<div class="charts">
		...
		<div class="userbox special" style="width: 33%;">
			<h2>用户持有品牌TOP5</h2>
			<!-- 排行 -->
			<e:if condition="${e:length(newApp.list)<=0}" var="select8">
								数据不存在！
			</e:if>
			<e:else condition="${select8 }">
				<ul class="ulrank">
		...
		<div class="userbox" style="width: 33%;">
			<h2>流量占收比</h2>
			<!-- charts图表预留位置 -->	
    		<div class="charts" width="450" height="200" id="charts1"></div>	
			<!-- //charts图表预留位置 -->
		</div>
	</div>
	<!-- //用户特征新增 -->
