        ...
        $(function(){
      	    $('#tt').tabs({
				onSelect:function(title,index){
					$('#con'+index).attr('src', $('#con'+index).attr('src'));
				}
			});
	        $(".EntryGroupLine input").bind("hover focus", function() {
	         ...
	         $("iframe").css({height:(window.screen.height-185)+'px'});
...
select RESOURCES_ID, RESOURCES_NAME, URL
			  FROM e_menu t
				 where t.parent_id = (select resources_id from e_menu where url = 'pages/terminal/usertrait/userFrame.jsp')
				   and RESOURCES_ID in
				       (select id
	...
	<div class="easyui-tabs searcharea" style="width:100%;" id="tt">
	    <e:forEach items="${RootMenuList.list}" var="item" indexName="index">
			...
			<iframe id="con${index }" src='<e:url value="${item.URL }"/>' width="100%"  style="position:relative;" frameBorder="0"></iframe>
