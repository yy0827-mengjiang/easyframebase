...
select RESOURCES_ID, RESOURCES_NAME, URL
			  FROM e_menu t
				 where t.parent_id = (select resources_id from e_menu where url = 'pages/terminal/market/marketFrame.jsp')
				   and RESOURCES_ID in
				       (select id
		...
		<div class="easyui-tabs searcharea" style="width: 100%;">
			<e:forEach items="${RootMenuList.list}" var="item" indexName="index">
				...
				<iframe id="con" src='<e:url value="${item.URL }"/>' width="100%"
					style="position: relative;" frameBorder="0"  ></iframe>
		</div>
		</e:forEach>
	</div>
