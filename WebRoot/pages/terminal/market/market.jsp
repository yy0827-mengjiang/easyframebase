...
<e:if condition="${sessionScope.UserInfo.ADMIN == '1'}">
	<e:set var="area_no">全国</e:set>
</e:if>
<e:if condition="${sessionScope.UserInfo.ADMIN != '1'}">
	<e:set var="area_no">1001</e:set>
</e:if>
...
<e:if condition="${area_no!='全国' }">
	...
	<e:q4o var="top1">
		<e:description>省份所对应的销量第一名的机型名称</e:description>
	     ...
	     WHERE  a."MODEL_NO" = b."MODEL_NO"  AND a."AREA_NO" = '${area_no}'
...
<e:if condition="${area_no=='全国'||area_no=='qg' }">
...
<e:q4o var="citynum">SELECT SUM(B."TERMINAL_NUM") terminalnum
		FROM "DM_M_TERM_MARKET_OVERVIEW" B 
		INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
		(C."AREA_LEVEL" ='一级' OR C."CITY_LEVEL"='一级') 
...
<e:q4o var="city2num">
	SELECT SUM(B."TERMINAL_NUM") terminalnum 
	FROM "DM_M_TERM_MARKET_OVERVIEW" B INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
 	C."CITY_LEVEL"='二级'AND (B."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
	AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
	<e:if condition="${param.checkoper!=null&&param.checkoper ne ''}">
		AND B."LINE_TYPE"=#checkoper#
	</e:if>
...
<e:q4o var="city3num">SELECT SUM(B."TERMINAL_NUM") terminalnum
		FROM
		"DM_M_TERM_MARKET_OVERVIEW" B INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
	 	C."CITY_LEVEL"='三级' AND (B."ACCT_MONTH"  BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
	<e:if condition="${param.checkoper!=null&&param.checkoper ne ''}">
		AND B."LINE_TYPE"=#checkoper#
	</e:if>
...
<e:q4l var="city1name">SELECT DISTINCT C."CITY_NAME" cityname
		FROM
		"DM_M_TERM_MARKET_OVERVIEW" B INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
		(C."AREA_LEVEL" ='一级' OR C."CITY_LEVEL"='一级') AND (B."ACCT_MONTH"  BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
	<e:if condition="${param.checkoper!=null&&param.checkoper ne ''}">
		AND B."LINE_TYPE"=#checkoper#
	</e:if>
...
<e:q4l var="city2name">SELECT DISTINCT C."CITY_NAME" cityname
		FROM 
		"DM_M_TERM_MARKET_OVERVIEW" B INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
		 C."CITY_LEVEL"='二级' AND (B."ACCT_MONTH"  BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
...
<e:q4l var="city3name">SELECT DISTINCT C."CITY_NAME" cityname
		FROM
			"DM_M_TERM_MARKET_OVERVIEW" B INNER JOIN "DIM_AREA_LEVEL" C ON B."AREA_NO"=C."AREA_NO" AND B."CITY_NO"=C."CITY_NO" AND 
		 C."CITY_LEVEL"='三级' AND (B."ACCT_MONTH"  BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
...
<e:q4l var="contract">	SELECT B.a,B.x FROM((SELECT '合约机' a,l.x
			FROM (SELECT
...
<e:q4l var="price">SELECT A."TERMINAL_NUM" TN,B."PRICE_NAME" SN FROM "DM_M_TERM_MARKET_OVERVIEW" A INNER JOIN "DIM_PRICES_PERIOD" B ON A."PRICE_NO"=B."PRICE_NO" 
		WHERE A."ACCT_MONTH"  BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm'), 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm')
	<e:if condition="${param.checkoper!=null&&param.checkoper ne ''}">
	...
	<e:if
		...
		ORDER BY A."TERMINAL_NUM" ASC LIMIT 5
</e:q4l>
...
<%-- <e:q4l var="columbrandTerminalnum">SELECT
			SUM (B."TERMINAL_NUM") y,
				B."ACCT_MONTH",
			round(
				SUM (B."TERMINAL_NUM") :: NUMERIC / (
		...
		WHERE
			B."BRAND_NO" = C ."BRAND_NO" AND (B."ACCT_MONTH" BETWEEN to_char(to_date('${acct_monthbefore}','yyyymm')-6, 'yyyymm')
		AND to_char(to_date('${acct_month}','yyyymm'), 'yyyymm'))
		<e:if condition="${param.checkoper!=null&&param.checkoper ne ''}">
			AND B."LINE_TYPE"=#checkoper#
		</e:if>
		<e:if condition="${param.area!=null&&param.area ne ''&&param.area ne 'qg'}">
			AND B."AREA_NO"=#area#
		</e:if>
		<e:if condition="${param.city!=null&&param.city ne ''&&param.city ne 'qxz'}">
			AND B."CITY_NO"=#city#
		</e:if>
			GROUP BY B."ACCT_MONTH",B."BRAND_NO",C."BRAND_NAME" ORDER BY y DESC LIMIT 5
</e:q4l> --%>
<%-- <e:q4l var="columbrandTerminalnum">SELECT sum(M.y) shuliang,M.brandname FROM (SELECT
			B."ACCT_MONTH" acctmonth, 
			B."BRAND_NO",
			C ."BRAND_NAME" brandname,
		B."TERMINAL_NUM" y
		FROM
		...
		WHERE
			B."BRAND_NO" = C ."BRAND_NO" AND B."ACCT_MONTH"=to_char(
				to_date('201610', 'yyyymm'),'yyyymm'))M WHERE M.acctmonth =to_char(
				to_date('201610', 'yyyymm'),
				'yyyymm'
...
<script type="text/javascript" language="JavaScript">
   		...
   		function doSearch(){
			...
			window.location.href='<e:url value="/pages/terminal/market/market.jsp"/>?area='+area+'&city='+city+'&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month+'&checkoper='+$('#line_type').val();
	    	} 
		...
		function lineType1(){
	    	...
	    	var acct_monthbefore= $('#acct_monthbefore').datebox('getValue').replace("-", "");
	    	var acct_month=$('#acct_month').datebox('getValue').replace("-", "");
	  	 	window.location.href='<e:url value="/pages/terminal/market/market.jsp"/>?area='+area+'&city='+city+'&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month+'&checkoper='+1;
	  	}
	  	//存网
		function lineType2(){
			var area=$('#area').val();
	    	...
	    	var acct_monthbefore= $('#acct_monthbefore').datebox('getValue').replace("-", "");
	    	var acct_month=$('#acct_month').datebox('getValue').replace("-", "");
		 	window.location.href='<e:url value="/pages/terminal/market/market.jsp"/>?area='+area+'&city='+city+'&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month+'&checkoper='+2;
		...
		function lineType3(){
	    	...
	    	var acct_monthbefore= $('#acct_monthbefore').datebox('getValue').replace("-", "");
	    	var acct_month=$('#acct_month').datebox('getValue').replace("-", "");
	  		window.location.href='<e:url value="/pages/terminal/market/market.jsp"/>?area='+area+'&city='+city+'&acct_monthbefore='+acct_monthbefore+'&acct_month='+acct_month+'&checkoper='+3;
	  	}
	 	$(function(){
	 		    // 初始化echarts图表
 /* function doQuery(){
            var dom = document.getElementById('pinpai');
	    ...
	    if('${area_no}' == '全国'||'${area_no}' == 'qg' ){
        	...
        	go_map(1,1);     //第一个参数是在网类型   ，第二个参数是省份NO   群国默认为1
        	
        }
        if('${area_no}' != '全国'){
        	go_map(1,'${area_no}');   //第一个参数是在网类型   ，第二个参数是省份NO   群国默认为1
        }
	    });
	     function go_map(line_type,index){
    				var info={}; 	   			
...
<body>
		...
		<div class="easyui-tabs searcharea" style="width:99%;height:auto;">
			<div title="地区">
				<!-- 查询条件 -->
				<div class="searchbox">
				...
				<div style="overflow:hidden; min-height: 560px;">
					<div class="map-market" style="width: 65%;">
						<span class="span-mapmarket"> <input id="line_type"
							...
							id="online" onclick="lineType2()">存网</a> <a
							...
							<e:if condition="${param.checkoper}==1">
								<div class="pricechart">
									<e:if condition="${e:length(contract.list)>0}" var="isfalse">
										<e:if condition="${e:length(contract.list)>0}" var="isfalse">
											<c:npie id="pie2" showexport="false" width="120" height="160"
												unit="" title="" percentage="false" legend="true" legendAlign="center"
												fontsize="2px" x='销量' dimension="a" distance="0"
												colors="['#33ffff','#00ff33']" tipfmt="0"
												items="${contract.list}" />
										</e:if>
									...
									<p>
										<e:if condition="${e:length(reactive.list)>0}" var="isfalse">
											<e:forEach items="${reactive.list}" var="item">
												<e:if condition="${item.reactiveshuliang}!=null && ${item.reactiveshuliang}!=''" var="isfalse">
						...
						<div class="market" style="width: 49.5%;">
							...
							<table cellpadding="0" cellspacing="0" class="TableMarket">
								...
								<tbody>
									<tr>
										<td>一线城市</td>
										<td><a href="javascript:void(4)"
											class="easyui-linkbutton" onclick="$('#w1').window('open')"><展开></a></td>
										<e:if
									...
									<tr>
										<td>二线城市</td>
										<td><a href="javascript:void(5)"
											class="easyui-linkbutton" onclick="$('#w2').window('open')"><展开></a></td>
										<e:if
									...
									<tr>
										<td>三线城市</td>
										<td><a href="javascript:void(6)"
											class="easyui-linkbutton" onclick="$('#w3').window('open')"><展开></a></td>
										<e:if
			...
			<div title="终端">
				...
				<div class="searchbox">
					<span class="spantext">省级</span> <select name="state"
						...
						required="true" /> <span class="spantext">至</span><input
						id="acct_month" required="true" /> <a class="easyui-linkbutton"
						href="javascript:void(0);" onclick="doQuery();">查询</a>
				</div>
				<!-- //查询条件 -->
				<!-- 终端 -->
					...
					<tbody>
						<tr>
							<td>
								...
								<div id="pinpai">
									<c:ncolumn id="column1" width="400" stacking="true"
										yaxis="title:,unit:台,color:#AA4643" showexport="false"
										dimension="brandname" y="name:,type:column"
										items="${columbrandTerminalnum.list}" height="300"
										title="品牌销量 " dataPointClick="dataPointClick"
										colors="['#01d6ff','#ff8500','#ffcf13','#86d438','#8945fe']" />
								</div> <!-- //品牌销量echarts -->
							</td>
							<td>
								...
								<div>
									<img
										src='<e:url value="/pages/terminal/resources/themes/base/images/boncLayout/img/img02_TableTerminal.jpg"/>'
										alt="">
								</div> <!-- //机型销量echarts -->
							</td>
							<td>
								...
								<div>
									<img
										src='<e:url value="/pages/terminal/resources/themes/base/images/boncLayout/img/img03_TableTerminal.jpg"/>'
										alt="">
								</div> <!-- //价格段echarts:标题写在echarts里 -->
							</td>
						</tr>
						<tr>
							<td>
								...
								<div>
									<%-- <img src='<e:url value="/pages/terminal/resources/resources/themes/base/images/boncLayout/img/img04_TableTerminal.jpg"/>' alt=""> --%>
								</div> <!-- //终端制式echarts -->
							</td>
							<td>
								<!-- cpu核数echarts:标题写在echarts里 -->
								<div>
									<%-- <img src='<e:url value="/pages/terminal/resources/themes/base/images/boncLayout/img/img05_TableTerminal.jpg"/>' alt=""> --%>
								</div> <!-- //cpu核数echarts -->
							</td>
							<td>
								<!-- 屏幕大小echarts:标题写在echarts里 -->
								<div>
									<%-- <img src='<e:url value="/pages/terminal/resources/themes/base/images/boncLayout/img/img06_TableTerminal.jpg"/>' alt=""> --%>
								</div> <!-- //屏幕大小echarts:标题写在echarts里 -->
							</td>
						</tr>
					</tbody>
				</table>
