...
<e:description>
	...
	例：
		<a:caselectnew id='area_no' childElementId="city_no" defaultValue="431" defaultItemValue="" defaultItemText="请选择">
			select area_no as "CODE",area_desc as "CODEDESC" from cmcode_area
		</a:caselectnew>
...
<script type="text/javascript">
	$("#${id}").combobox({
	    ...
	    onLoadSuccess: function () {
	    	setDefault${id}();
	    	var v_${id}=$("#${id}").combobox("getValues");
	    	$("#${name}").val(v_${id});
        }
	   	,onSelect:function(record){
	   		var nameValue=$("#${id}").combobox("getValues");
	   		$("#${name}").val(nameValue);
		...
		<e:if condition="${multiple=='true'}">
			,onUnselect:function(record){
				var valueArray=$("#${id}").combobox("getValues");
				var realValues='';
				if(valueArray!=null){
					for(var i=0;i<valueArray.length;i++){
						if(valueArray[i]!=''&&valueArray[i]!=null&&valueArray[i]!=undefined&&valueArray[i]!='null'&&valueArray[i]!=record.value){
							realValues+=','+valueArray[i];	
						}
					}
				}
				if(realValues!=""){
					realValues = realValues.substr(1,realValues.length);
				}
				$("#${name}").val(realValues);
		   		if(window["select_${childElementId}"]!=undefined){//change child element
