	...
	var paramMap={};
		...
		$.ajax({
		      ...
		      success : function(data1){
				 ...
				 $.ajax({
				      type : "post",  
				      url : (window["appBase"]==undefined?('<e:url value="/"/>'.substring(0,'<e:url value="/"/>'.lastIndexOf('/'))):window["appBase"])+'/rotaData.e',  
				      data : paramMap,  
				      async : false,  
				      dataType:"json",
		...
		<e:if condition="${download!=null && download!=''}">
			...
			function download_exec_${id}(epath,cmenuid){
				...
				$.post(table_${id}_url+down_para_${id}+'${downArgs}',qParams,function(data){
					...
					if(epath == 'excel'){
						eurl = "/downCtableEC.e";
						$export = $('<form id=export${id} method=post action="'+(window["appBase"]==undefined?('<e:url value="/"/>'.substring(0,'<e:url value="/"/>'.lastIndexOf('/'))):window["appBase"])+'/downCtableEC.e">'+form_input+'<input type=hidden name=dataSourceName id=dataSourceName value=${extds }/><input type=hidden name=filename /><input type=hidden name=columns /></form>').appendTo('body');
					}else if(epath == 'xlsx'){
						eurl = "/downCtableXlsxEC.e";
						$export = $('<form id=export${id} method=post action="'+(window["appBase"]==undefined?('<e:url value="/"/>'.substring(0,'<e:url value="/"/>'.lastIndexOf('/'))):window["appBase"])+'/downCtableXlsxEC.e">'+form_input+'<input type=hidden name=dataSourceName id=dataSourceName value=${extds }/><input type=hidden name=filename /><input type=hidden name=columns /></form>').appendTo('body');
					}else{
						eurl = "/downCtablePdfEC.e";
						$export = $('<form id=export${id} method=post action="'+(window["appBase"]==undefined?('<e:url value="/"/>'.substring(0,'<e:url value="/"/>'.lastIndexOf('/'))):window["appBase"])+'/downCtablePdfEC.e">'+form_input+'<input type=hidden name=dataSourceName id=dataSourceName value=${extds }/><input type=hidden name=filename /><input type=hidden name=columns /></form>').appendTo('body');
					}
