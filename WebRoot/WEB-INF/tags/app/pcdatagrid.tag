...
<%@ attribute name="rotadim" required="false" %> 										    <e:description>旋转列名称</e:description>
<%@ attribute name="kpiCol" required="false" %> 										    <e:description>指标列名称</e:description>
<%@ attribute name="valCol" required="false" %> 										    <e:description>指标值列名称</e:description>
<%@ attribute name="dimDesc" required="false" %> 										    <e:description>维度描述</e:description>
...
<table id="${id}"<e:forEach items="${dynattrs}" var="item"> ${item.key}="${item.value}"</e:forEach>>
  <e:if condition="${rotadim!=null&&rotadim ne ''}" var='rotateIf'>
     <thead id='${id}_head'></thead>
  </e:if>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		<e:if condition="${rotadim==null||rotadim eq ''}" var='rotateIf'>
			initConfig_${id}(null);
	    </e:if>
		<e:else condition="${rotateIf}">
				var paramMap={};
				paramMap.rotaDimStr='${rotadim}';
				$.ajax({
				      type : "post",  
				      url : (window["appBase"]==undefined?('<e:url value="/"/>'.substring(0,'<e:url value="/"/>'.lastIndexOf('/'))):window["appBase"])+('${url}'.indexOf("/")==0?'${url}':'/${url}'),  
				      data : paramMap,  
						 ...
						 $.ajax({
						      ...
						      success : function(data2){
		                        ...
		                        if(data2.rows.length>0){
		                        	...
		                        	tbody+="<th field='${rotadim}' align='center'>${dimDesc}</th>";
		    						for(var key in obj){
		    							if(key!='${rotadim}'){
		    								tbody+="<th field='"+key+"' align='center'>"+key+"</th>";
		    							}
		    						}
		                        }
		                        tbody+="</tr>";
		                        $("#${id}_head").html(tbody);
		                        initConfig_${id}(data2);
						      }
						  }); 
				      }
				});
