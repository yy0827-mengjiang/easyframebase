...
var ${id} = {
	...
	open:function(){
		$("#caculateDialog_${id}").dialog('open').dialog('resize',{width:'750px',height:'400px',left:$(window).width()-1150});
	...
	initCandidateCol:function(){//初始化候选列
		kpiSelectorService.getDataFromKpiLibrary(${id}.getCurDs(),'kpi',StoreData.xid,function(data,exception){
			var kpiData = data.kpi;
			var treeData = [{id: '-1',text: "候选列",state: "closed",children: []}];
			$.each(kpiData,function(index,item){
				var groupObj = {id:"group_"+index,text:item.title,children:[]};
				$.each(item.children,function(index,kpiItem){
					var kpiObj = {};
					kpiObj.id = kpiItem.id;
					kpiObj.text = kpiItem.desc;
					kpiObj.attributes = {column:kpiItem.column};
					if(kpiItem.children!=undefined&&kpiItem.children!=null){
						kpiObj.children = new Array();
						$.each(kpiItem.children,function(index,kpiChild){
							kpiObj.children.push({
								id:kpiChild.id,
								text:kpiChild.desc,
								attributes:{column:kpiChild.column}
							});
						});
					}
					groupObj.children.push(kpiObj);
				});
				treeData[0].children.push(groupObj);
			}); 
			$("#candidateColList_${id}").tree({
				...
				onBeforeDrop : function (target, source, point){
			        if (point === 'append'){
			 		...
			 		var rootNode = $("#candidateColList_${id}").tree('getRoot');
			 		$("#candidateColList_${id}").tree('expand',rootNode.target);
			 	}
			});
			$("#candidateColList_${id}").tree('loadData',treeData); 
		});
	},
	initCaculateCol:function(){//初始化计算列
		var extPath = appBase+"/getKpiExtColumnsJson.e?report_id="+StoreData.xid;
				...
				$.each(parmaArr,function(index,item){
					$('#param_'+time+"_"+index).droppable({
						...
						onDrop: function(e,source){
							...
							$("span[name='"+$(this).attr("name")+"']").attr('kpiId',$(source).attr("node-id"));
							$(this).css('background','#fff');
						}
					});
				});
				...
				var paramList = data.paramList;//回显参数列表
				 if(paramList!=null&&paramList!=undefined){
	            	 $.each(paramList,function(index,item){
	            		var paramText = $("#candidateColList_${id}").tree("find",item.value).text;
	      				$("span[name='param_"+item.name+"']").text(paramText);
...
function okBtnClick_${id}(){
	...
	<a href="javascript:void(0)" class="openKpi" data-options="" onclick="openCaculateDialog_${id}()" style="margin-left:3px;display:none;">计算列</a>
	...
	<div id="caculateDialog_${id}" class="easyui-dialog" title="计算列" data-options="closed:true,buttons:[{text:'确定',handler:okBtnClick_${id}},{text:'取消',handler:function(){${id}.close();}}]" style="width:750px;height:390px;padding:5px">
	        ...
	        <div class="easyui-layout" style="width:720px;height:310px;">
			    <div data-options="region:'west',title:'',border:false" style="width:200px;">
				    <div id="colTab_${id}" class="easyui-tabs" style="width:195px;height:310px">
							<div title="计算列" style="padding:5px">
							    <ul id="canculateColTree_${id}" style="width:170px;border:none;overflow-y:auto; overflow-x:hidden">
							    </ul>
							</div>
							<div title="候选列" style="padding:5px;height:280px">
							    <ul id="candidateColList_${id}" style="width:170px;border:none;overflow-y:auto; overflow-x:hidden">
			    ...
			    <div data-options="region:'center',title:'',border:false">
			        <div style="float:left;padding-top:10px;padding-left:10px;">
						  <input id="columnName_${id}" style="width:180px;height:19px;" placeholder="计算列名称" />
						  <select class="easyui-combobox" id="formulaComb_${id}" data-options="valueField:'ID',textField:'VALUE',editable:false" style="width:230px;"></select>
	...
	<!-- 自定义公式对话框 -->
	<div id="customFormulaDialog_${id}" class="easyui-dialog" title="自定义公式" data-options="closed:true" style="width:400px;height:165px;padding:10px">
