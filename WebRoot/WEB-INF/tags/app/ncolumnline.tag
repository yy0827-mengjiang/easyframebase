...
<%@ attribute name="legendAlign" required="false" %>      									<e:description>图例水平对齐:left、center、right</e:description>
<%@ attribute name="legendValign" required="false" %>      									<e:description>图例垂直对齐：能取"top", "middle" or "bottom"之一</e:description>
<%@ attribute name="rotation" required="false" %>      								<e:description>坐标轴文字旋转角度，默认为-45度</e:description>
<%@ attribute name="offsetX" required="false" %>      								<e:description>横坐标轴文字水平偏移量</e:description>
<%@ attribute name="offsetY" required="false" %>      								<e:description>横坐标轴文字垂直偏移量</e:description>
...
<e:if condition="${legendAlign==null || legendAlign == ''}">
	<e:set var="legendAlign">center</e:set>
</e:if>
<e:if condition="${legendValign==null || legendValign == ''}">
	<e:set var="legendValign">bottom</e:set>
</e:if>
<e:if condition="${rotation==null || rotation == ''}">
	<e:set var="rotation">-45</e:set>
</e:if>
...
<script type="text/javascript">	
	var options_${id}_${TmpUUID} = {
		...
		xAxis: {
			...
			labels: {
				rotation: ${rotation},
				<e:if condition="${offsetX!=null&&offsetX!=''}">
				 	x:${offsetX},
				</e:if>
				<e:if condition="${offsetY!=null&&offsetY!=''}">
				 	y:${offsetY},
				</e:if>
				align: 'center',
				style: {
		...
		<e:forEach items="${dynattrs}" var="item">
	            ...
	            if(series_${index}.type=="column"){
	              options_${id}_${TmpUUID}.series.push(series_${index});
	               ...
	               seriesArr.push(series_${index});
	            }
        	</e:forEach>
        	//var chart_${id} = new Highcharts.Chart(options_${id}_${TmpUUID});
        	for(var i=0;i<seriesArr.length;i++){
        	   options_${id}_${TmpUUID}.series.push(seriesArr[i]);
        	}
