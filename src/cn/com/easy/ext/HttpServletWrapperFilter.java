 ...
 * @since 3.0
 */
public final class HttpServletWrapperFilter extends AbstractConfigurationFilter {
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain filterChain) throws IOException, ServletException {
    	...
    	final AttributePrincipal principal = retrievePrincipalFromSessionOrRequest(request,session);
        if(principal!=null&&session!=null){
        	try {
        		//模拟登录实现
        		DefaultLogin dLogin = new DefaultLogin();
        		Map uMap = dLogin.PortalLogin(principal.getName(), request);
        		if(uMap == null){
   				 //统一退出，重定向或转发
    				//response.sendRedirect("/cas/logout");
    				request.getRequestDispatcher("/cas/logout").forward(request, response);
    				return;
        		}
			} catch (Exception e) {
				 e.printStackTrace();
    ...
    protected AttributePrincipal retrievePrincipalFromSessionOrRequest(final HttpServletRequest request,final HttpSession session) {
