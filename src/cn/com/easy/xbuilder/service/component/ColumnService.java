...
@Service
public class ColumnService extends ComponentBaseService {

	/* 删除计算列前检查计算列是否已经被其他组件使用 */
	@SuppressWarnings("unchecked")
	public String checkExtColRef(String reportId, String containerId,String componentId, String jsonString){
		Map<String, String> element = (Map) Functions.json2java(jsonString);
		String extcolumnid = element.get("extcolumnid");
		return super.checkCaculateColumnReference(reportId, containerId, componentId, extcolumnid);
	}
	...
	/* 设置标题 */
	@SuppressWarnings("unchecked")
	public void setTitle(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
		Map<String, String> element = (Map) Functions.json2java(jsonString);
	...
	/* 设置是否堆叠显示 */
	@SuppressWarnings("unchecked")
	public void setStacking(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
		Map<String, String> element = (Map) Functions.json2java(jsonString);
		component.setStacking(element.get("stacking"));
		saveToXmlByObj(reportObj);
		// System.out.println(validateAllChartComponent(reportObj,component));
	}
	...
	/* 设置图例 */
	@SuppressWarnings("unchecked")
	public void setLegend(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 设置图标指标的颜色 */
	@SuppressWarnings("unchecked")
	public void setColors(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 是否设置宽高 */
	@SuppressWarnings("unchecked")
	public void showSetHW(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 设置宽度 */
	@SuppressWarnings("unchecked")
	public void setWidth(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 设置高度 */
	@SuppressWarnings("unchecked")
	public void setHeight(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 设置数据集 */
	@SuppressWarnings("unchecked")
	public void setDataSourceId(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 设置排序列 */
	@SuppressWarnings("unchecked")
	public void setSortField(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
		...
		saveToXmlByObj(reportObj);
		// 设置排序方式时调用生成sql方法
		if (element.get("ordKpiId") != null && oldSortType != null
				&& !oldSortType.equals(element.get("sortType"))) {
	...
	/* 设置维度列 */
	@SuppressWarnings("unchecked")
	public void setDimField(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 刻度设置 */
	@SuppressWarnings("unchecked")
	public void setYaxis(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
		...
		for (int i = 0; i < yaxisList.size(); i++) {
			Map<String, String> tempMap = (Map<String, String>) yaxisList
					.get(i);
	...
	/* 指标设置 */
	@SuppressWarnings("unchecked")
	public void setKpi(String reportId, String containerId, String componentId,
			String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	/* 取组件json数据 */
	@SuppressWarnings("unchecked")
	public String getComponentJsonData(String reportId, String containerId,
			String componentId, String jsonString) {
		Report reportObj = XContext.getEditView(reportId);
		Component component = getOrCreateComponet(reportObj, containerId,
				componentId);
	...
	public String validateComponent(Report report, Component component) {
		...
		if ("".equals(component.getTitle())) {
			stringBuilder.append("||||柱图：未配置组件“标题”；<br/>\n");
		}
		if ("".equals(component.getDatasourceid())
				|| component.getDatasourceid() == null) {
			stringBuilder.append("||||柱图：未配置组件“数据源”；<br/>\n");
		} else {
			if (report.getDatasources() != null
					&& report.getDatasources().getDatasourceList() != null) {
				...
				for (Map tempDataSetModel : dataSetModelList) {
					if (String.valueOf(tempDataSetModel.get("id")).substring(1)
							.equals(component.getDatasourceid())) {
						selectedDataSetModel = tempDataSetModel;
					}
				}
				if (selectedDataSetModel == null) {
					stringBuilder.append("||||柱图：组件配置的数据源已不存在；<br/>\n");
				}
			}
		...
		if (null == component.getXaxis()) {
			stringBuilder.append("||||柱图：未设置组件的“排序列”和“维度列”；<br/>\n");
		} else {
			if ("".equals(component.getXaxis().getSortfield())&&"".equals(component.getXaxis().getSortExtField())) {
				stringBuilder.append("||||柱图：未设置组件的“排序列”；<br/>\n");
			}

			if ("".equals(component.getXaxis().getDimfield())&&"".equals(component.getXaxis().getDimExtField())) {
				stringBuilder.append("||||柱图：未设置组件的“维度列”；<br/>\n");
			}

			if ("1".equals(dsType) && selectedDataSetModel != null) {
				if ((!validateDatasourceField(selectedDataSetModel, component
						.getXaxis().getSortfield()))
						&& (!("".equals(component.getXaxis().getSortfield())))) {
					stringBuilder.append("||||柱图：组件的排序列“"
							+ component.getXaxis().getSortfield()
							+ "”已不存在；<br/>\n");
				}
				if ((!validateDatasourceField(selectedDataSetModel, component
						.getXaxis().getDimfield()))
						&& (!("".equals(component.getXaxis().getDimfield())))) {
					stringBuilder.append("||||柱图：组件的维度列“"
							+ component.getXaxis().getDimfield()
							+ "”已不存在；<br/>\n");
				}
			}
		}
		...
		if (null == yaxisList) {
			stringBuilder.append("||||柱图：未设置组件的“刻度”；<br/>\n");
		} else {
			// int i=0;
		...
		List<Kpi> kpiList = component.getKpiList();
		if (null == kpiList) {
			stringBuilder.append("||||柱图：组件未设置任何指标；<br/>\n");
		} else {
			for (Kpi tempKpi : kpiList) {
				if ("".equals(tempKpi.getField())&&"".equals(tempKpi.getExtcolumnid())) {
					stringBuilder.append("||||柱图：未对组件的“指标设置”下的“指标列”进行设置；<br/>\n");
				}

				if (null != selectedDataSetModel && "1".equals(dsType)) {
					if ((!validateDatasourceField(selectedDataSetModel,
							tempKpi.getField()))
							&& (!("".equals(tempKpi.getField())))) {
						stringBuilder.append("||||柱图：组件的指标列“" + tempKpi.getField()
								+ "”已不存在；<br/>\n");
					}
				}

				if ("".equals(tempKpi.getName())) {
					stringBuilder.append("||||柱图：未设置组件的“指标名”；<br/>\n");
				}

				if ("".equals(tempKpi.getType())) {
					stringBuilder.append("||||柱图：未设置组件的“指标类型”；<br/>\n");
				}
			}
		}
	...
	/* 生成组件标签代码 */
	@SuppressWarnings("unchecked")
	public String getComponentTagCode(String reportId, String containerId,
			String componentId, String actionUrl) {
		...
		if (!("".equals(component.getAttrenable()))
				&& "true".equals(component.getAttrenable())) {
		...
		for (YAxis tempYAxis : yaxisList) {
			...
			if (!("".equals(tempYAxis.getColor()) || "#fff".equals(tempYAxis
					.getColor()))) {
				tempStringBuilder
						.append("yColor:" + tempYAxis.getColor() + ",");
			}
			 ...
			 */
			stringBuilder.append(tempStringBuilder.substring(0,
					tempStringBuilder.length() - 1));
		...
		Kpi tempKpi : kpiList) {
			...
			if (!("".equals(tempKpi.getField()))) {
				...
				if (!("".equals(tempKpi.getColor()) || "#fff".equals(tempKpi
						.getColor()))) {
